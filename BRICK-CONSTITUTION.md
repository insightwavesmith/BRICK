# BRICK 헌법 (0702 Smith 비준 · 0705 개정 · 0706 개정 · 0708 개정)
...
개정 기록(0706 Smith 비준): 판정 권한 배분 문구 — 3축 절·진단 5단 ⑤의 "품질+성공 판단 =
사람"을 "품질+성공 판정 권한은 Smith가 배분한다(기본 소재: 사람)"로 개정(후보 등재:
goal doc §G3 개방, 0705 심야 — 사람 승인이 상수가 아니라 권한 소재가 Smith의 주권적·
위임가능 결정. 위임 하 판정은 근거·반전 경로를 투명하게 남긴다).
개정 기록(0708 Smith 비준): C2 이후 active physical roots를 헌법에 등재한다. 물리 루트는
3축과 support/project 증거 위치를 드러낼 뿐, 새로운 축·판단권·Movement 권한을 만들지 않는다.
...
- **support는 사실만 기록하고 아무것도 판단하지 않는다.** 충분성+Movement 판단 = Link 게이트.
  품질+성공 판정 권한은 Smith가 배분한다(기본 소재: 사람). (0706)
...
C2 이후 Python package root는 물리 `brick_protocol/` tree다. Legacy top-level `brick/`,
`agent/`, `link/`, `support/` roots는 active import roots가 아니다.
...
5. **3축 헌법대로** — support는 사실, Link 게이트는 충분성+Movement, 품질+성공 판정
   권한은 Smith가 배분(기본 소재: 사람).
