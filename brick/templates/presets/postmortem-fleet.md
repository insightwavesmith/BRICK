...
selection_hint: Use when a completed incident, repair wave, or decision chain needs paired Codex and Gemini postmortem review lenses before closure synthesis.
steps:
...
Mixed-provider postmortem route: inspect the postmortem boundary, fan out paired Codex and Gemini code, axis, and evidence-integrity lenses, then fan in to Codex closure synthesis. The fleet declares graph shape only; the engine may still walk the declared graph sequentially.
