...
description: BRICK 빌딩 발주 한 스킬 — PHASE 1 task 본문 쓰기 → 경로 결정(프리셋/어셈블·자율/감독) → PHASE 2 발사 → PHASE 3 홀드 분류(걸리면). 새 빌딩 발주, 수리·기능·부검 task를 쓸 때 이 절차로. 모양 사이징은 building-sizing-method 스킬.
---

# BRICK 발주 (운영자 표준, 3-PHASE)

> 항상 **`/Users/smith/projects/BRICK`(제품)에서** 발사·측정·코드수정. 세션 워크트리(brick-protocol)는 **동결 박물관** — 거기선 다 금지(isolation:worktree 에이전트도 박물관을 받으니 제품 작업에 격리 워크트리 쓰지 마라).
...
│     → PRESET 경로 (PHASE 2-A). render_preset_ranking_packet로 랭킹(advisory — 내가 고름).
│
└─ 새 모양(팬아웃·팬인·병렬·다단), 맞는 프리셋 없음
      → 먼저 building-sizing-method 스킬로 모양 산출 → ASSEMBLE 경로. 누가 보나로 가른다:
        ├─ 사람이 머지 전 승인(감독)  → assemble(gates=("human-review",))
        └─ 골까지 무인(Smith 기본)    → §AUTO 직행 (PHASE 2-B)
```

발주 공개 동사 = **프리셋(`run_building_intake`) 아니면 어셈블(`assemble`)+발사(`launch_assembled_building`).**
`driver_public_intake_seal` 체커는 raw 뒷문(`run_composed_graph_intake`)을 driver.py 공개표면에서
막는다 — assemble은 assembly.py 소유로 **체커 범위 밖**. 3번째로 사고하지 마라.
...
# PHASE 2-A — PRESET 경로 (run_building_intake)

```python
# cd /Users/smith/projects/BRICK && uv run python3 -c "..."
from brick_protocol.support.connection.building_design_toolkit import render_preset_ranking_packet
pkt = render_preset_ranking_packet("<task 요지 한 줄>", repo_root="/Users/smith/projects/BRICK")
# 토큰겹침 점수 desc. ADVISORY(자동선택 아님) — 내가 골라 chain_preset_ref로 박는다. 카탈로그=brick/templates/presets/
```
intent → `run_building_intake(intent, repo_root=BRICK, adapter_cwd=<전용 워크트리>, adapter_timeout_seconds=3600)`. **adapter_cwd 절대 누락 금지**(누락 시 라이브 제품 트리에 작업).
```python
...
# PHASE 2-B — §AUTO 자율(무인) 직행 — launch_assembled_building 한 동사

`assemble`로 모양을 조립하고 **`launch_assembled_building`** 한 동사로 발사한다. 이 동사가 옛
**4지뢰를 다 흡수**한다(plan을 디스크에 기록=①, 베슬 project_ref에서 buildings_root_for로
루트 유도=②, 워크트리 샌드박스 안 실행=③, goal-path `build`와 이름 다름=④, 강제 사람게이트 없음).

```python
# cd /Users/smith/projects/BRICK && uv run python3 -c "exec(open('/tmp/launch.py').read())"
...
result = launch_assembled_building(
    ...
    # report_env 생략 ⇒ ~/.brick/report.env 자동로드 ⇒ 슬랙·대시보드 발화
)
