...
KIND는 **데이터**다 — `brick/spec.BrickSpec` 코드를 안 고친다. 새 casting 다이얼은 `CASTING_FIELDS`로
...
rec = scaffold_brick_kind(
    ...
    agent_object_hint_ref="agent-object:<slug>",  # 선택. agent/objects/<slug>.yaml 실재해야
...
- **primary required_return_template_refs[0] == `brick/templates/bricks/<kind>/return.yaml`** (정확히).
...
정확한 위치를 알려준다. `support/checkers/check_package_path_admission.py`의 `TEMPLATE0_BRICK_KINDS`
...
쓰게 하려면 프리셋에 스텝을 추가한다 (`brick/templates/presets/<name>.md`):
```yaml
steps:
  - step_template_ref: building-step-template:<kind>
    brick_spec_ref: brick/templates/bricks/<kind>/brick.md
    target_word: <…>
```
...
⚠️ 체커 **스크립트를 직접** 돌리면(`python3 support/checkers/check_bricks_spec_completeness.py`)
...
PYTHONPATH=support/import_identity uv run python3 \
  support/checkers/check_profile.py --profile core                       # exit 0
# structure_template_integrity: 카탈로그 orphan_physical_template 없음 + admission.
PYTHONPATH=support/import_identity uv run python3 \
  support/checkers/check_profile.py --profile structure_template_integrity   # exit 0
```
...
## 알아둘 것
- KIND는 데이터 — `brick/spec.py` 안 고친다. `BRICK_ROW_ALLOWED_KEYS`는 새 brick-ROW *키*를
  더할 때만 바뀐다(보통 안 바뀜).
