...
def _assert_loader_and_shape(repo: Path) -> int:
    from support.operator import resume_declaration as mod
...
def _assert_cli_and_profile_pins(repo: Path) -> int:
    cli_text = (repo / "support/operator/cli.py").read_text(encoding="utf-8")
    required_cli = (
    ...
    profile = repo / "support/checkers/profiles/resume_declaration.yaml"
