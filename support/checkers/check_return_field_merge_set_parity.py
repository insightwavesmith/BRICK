...
``_merge_structured_return_fields`` in support/connection/adapter_grant_policy.py.
Every field in that merge-set MUST also be in ``_RETURN_LIST_FIELDS``
(support/connection/agent_adapter.py) so the upstream normalizer
...
# structural evidence.
_AGENT_ADAPTER_REL = Path("support/connection/agent_adapter.py")
_GRANT_POLICY_REL = Path("support/connection/adapter_grant_policy.py")
