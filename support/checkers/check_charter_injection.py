...
in a temp tree (``agent/`` symlinked to the real axis so role resolution is
...
def _make_probe_repo(real_repo: Path, tmp_root: Path) -> Path:
    """A temp repo whose ``agent/`` axis is the real one (real role resolution)
    ...
    # hooks/tool_policies resolve for real; the renderer reads only repo/agent.
    os.symlink(real_repo / "agent", probe_repo / "agent", target_is_directory=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if failures:
        ...
        print(
            ...
            "README charter (support/connection/agent_resources._charter_resources) for "
            "every role; the call sites (support/operator/run.py, walker_kernel.py) must "
