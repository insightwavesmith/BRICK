...
(``brick/templates/bricks/<kind>/brick.md`` + ``return.yaml``) with the
...
DATA, not a code enum — ``brick/spec.BrickSpec`` needs no edit and a new casting
dial flows in through ``CASTING_FIELDS`` automatically.

The skill (brick/templates/skills/make-a-brick) calls ``scaffold_brick_kind`` and
...
_BRICKS_DIR = "brick/templates/bricks"
_CATALOG_REL = "brick/templates/shapes/catalog.yaml"
_RETURN_FILENAME = "return.yaml"
_BRICK_FILENAME = "brick.md"
_TRANSITION_CONCERN_REF = "brick/templates/bricks/transition-concern-return.yaml"
...
_ADMISSION_CHECKER_REL = "support/checkers/check_package_path_admission.py"
