...
Everything else — `support/`, checkers, docs, projections, runtime, providers — is
...
The path admission checker (`support/checkers/check_package_path_admission.py`)
...
support/docs/`. The legacy/runtime owner roots are rejected with a different
...
Documentation lives under `support/docs/<admitted subdir>`. The admitted doc
...
`support/docs/spec/`, `support/docs/reviews/`, `support/docs/references/`, and
`support/docs/projection/`. (This page itself is a new `.md` under
`support/docs/references/`, which is why no checker change was required to add it.)
...
- **Agent Object resources.** `support/connection/agent_resources.py` defines
  `_FORBIDDEN_AGENT_OBJECT_KEYS`, which includes `session_id`,
  `provider_session_id`, `credential_body`, `setup_token`, and
  `setup_token_value`. An Agent Object (`agent/objects/*.yaml`) carrying any of
...
- **Building evidence files.** `support/checkers/check_building_lifecycle_path_shape.py`
...
- **Adapter source bodies.** `support/connection/agent_adapter.py` actively redacts
...
- `brick_protocol.support.operator.run` records "what was received, what was
  ...
  `support/docs/references/quickstart.md`), and states that evidence "is not source
...
- Every resolver in `support/connection/agent_resources.py` returns
...
it does not belong in the support/truth layer. It belongs in a separate,
...
Axis meaning is owned by the axis. Code under `support/` reads and records; it does
