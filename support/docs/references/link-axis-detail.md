...
Every claim below is grounded in the Link contract projections (`link/movement.yaml`, `link/gate.yaml`, `link/transfer.yaml`, `link/carry.yaml`, `link/transition.yaml`) and the constitution (`AGENTS.md`).
...
Each `link/*.yaml` is a `projection_only` contract that declares the public fact a Link module owns, its allowed fields, and its forbidden ownership.

## The gate measures sufficiency, not quality

`GateFact` (`link/gate.yaml`) records Link-side **sufficiency** for public facts. Its fields are:
...
Two related concepts in the matrix deliberately have **no** GateFact literal (`gatefact_literal: none`): `fan-in-wait-all` lives in declared graph fan-in observation / frontier, and `portfolio-policy` lives in `support/operator/driver.py` declared portfolio policy. They are not new gate authority.

## Movement literals are exactly forward | reroute

After sufficiency is known, a declared Link row records Movement. The Movement literals are **exactly two** (`link/movement.yaml`, `movement_literals`; confirmed in `AGENTS.md`, Link Axis):
...
Their meaning (`link/movement.yaml` `connection_route_rule`; `AGENTS.md`):
...
`MovementFact` (`link/movement.yaml`) carries:
...
`TransferFact` (`link/transfer.yaml`) describes a transfer between a source and target boundary:
...
`CarryFact` (`link/carry.yaml`) describes which facts carry forward to the next step, **preserving the axis that originally owned them**:
...
`TransitionFact` (`link/transition.yaml`) is the connection-level fact between Brick boundaries:
...
- The `link/*.yaml` files are **contract projections** (`projection_only: true`), not the executable Link modules. The executable surfaces are `link/*.py`; this page describes the declared contracts, not a verified runtime.
