...
> ⚠️ **이건 축 코드가 아니라 문서다.** `brick/agent/link/support` 로직·체커에 0 영향. `grep grounding`으로 재발견하기 위한 노트. (축 코드에 심으면 off-axis 노이즈·second-spec 냄새 → 일부러 문서로만 둠.)
...
→ **`support/docs/references/brick-grounding-map.md`** (별도 파일, ~70KB)
