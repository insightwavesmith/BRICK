...
_재검증 2026-06-11 · HEAD a000db0 · 67개 재파생 grep 전수 재실행: 48 그대로 / 19 드리프트 수리 — 18개는 토큰 verbatim 그대로 파일만 이사(evidence_assembly 분해 → support/recording/* ; building_operation → native_dispatch)라 경로·현재 위치만 재지정, 1개(link/gate.py 결합 게이트)는 게이트 시퀀스 개편(739b6d2)으로 심볼 이동 — 해당 앵커에 드리프트 기록 명시. 수리 후 재파생 67/67 재확인 + 프로젝트 그릇 앵커 1개 추가(아래 9절, live 검증)._
...
- **observe returned-value against required fields** — _anchor_
  ...
  - 재파생: `grep -nF 'adapter returned value is available for Brick comparison observation' brick/comparison.py`
  - 현재 위치(live 2026-05-30): `brick/comparison.py:128`  (matches: 1)
  - 근거:
    ```
    comparison_evidence = [
...
- **encode required/observed/missing field sets as evidence-line coordinates** — _anchor_
  ...
  - 재파생: `grep -nF '"missing_return_fields: " + ", ".join(missing_fields)' brick/comparison.py`
  - 현재 위치(live 2026-05-30): `brick/comparison.py:140`  (matches: 1)
  - 근거:
    ```
...
- **decide observed_match_kind from presence of missing fields** — _derive_
  ...
  - 재파생: `grep -nF 'observed_match_kind = "missing"' brick/comparison.py`
  - 현재 위치(live 2026-05-30): `brick/comparison.py:145`  (matches: 1)
  - 근거:
    ```
            if missing_fields:
...
- **made_changes / no_changes_reason satisfaction equivalence** — _derive_
  ...
  - 재파생: `grep -nF 'if field_name == "made_changes" and "no_changes_reason" in returned_value:' brick/comparison.py`
  - 현재 위치(live 2026-05-30): `brick/comparison.py:122`  (matches: 1)
  - 근거:
    ```
...
- **parse evidence line back into field names (reverse grounding accessor)** — _pointer_
  ...
  - 재파생: `grep -nF 'def fields_from_evidence(self, prefix: str) -> tuple[str, ...]:' brick/comparison.py`
  - 현재 위치(live 2026-05-30): `brick/comparison.py:159`  (matches: 1)
  - 근거:
    ```
...
- **validate observed_match_kind against the closed vocabulary** — _proof-limit_
  ...
  - 재파생: `grep -nF '_OBSERVED_MATCH_KINDS: tuple[str, ...] = ("matched", "missing", "mismatched", "unknown")' brick/comparison.py`
  - 현재 위치(live 2026-05-30): `brick/comparison.py:12`  (matches: 1)
  - 근거:
    ```
...
- **compute the declared required-field set (Brick shape + Link gate-derived union)** — _anchor_
  ...
  - 재파생: `grep -nF 'return gate_required_return_fields(gate_refs, base)' support/operator/plan_validation.py`
  - 현재 위치(live 2026-05-30): `support/operator/plan_validation.py:547`  (matches: 1)
  - 근거:
    ```
...
- **Link-owned gate->required-fields mapping** — _derive_
  ...
  - 재파생: `grep -nF 'def gate_required_return_fields(' link/gate.py`
  - 현재 위치(live 2026-05-30): `link/gate.py:107`  (matches: 1)
  - 근거:
    ```
...
- **project comparison fact into the traceable Brick claim-fact evidence record** — _pointer_
  ...
  - 재파생: `grep -nF '"comparison_observation": "contract observation only; not success judgment",' support/recording/claims_brick.py`
  - 현재 위치(live 2026-06-11, 재파생 재실행): `support/recording/claims_brick.py:93`  (matches: 2; also :118)
  - 근거:
    ```
...
- **Canonical raw-ref minter (raw:{kind}:NN)** — _anchor_
  ...
  - 재파생: `grep -nF 'return f"raw:{kind}:{index:02d}"' support/operator/primitives.py`
  - 현재 위치(live 2026-05-30): `support/operator/primitives.py:454`  (matches: 1)
  - 근거:
    ```
...
- **Event-type to axis dispatch (brick/agent/link)** — _derive_
  ...
  - 재파생: `grep -nF 'def _event_raw_ref(event_type: str, index: int) -> str:' support/operator/primitives.py`
  - 현재 위치(live 2026-05-30): `support/operator/primitives.py:457`  (matches: 1)
  - 근거:
    ```
...
- **Graph-edge raw-ref minter (raw:link-graph:NN:<edge-slug>)** — _anchor_
  ...
  - 재파생: `grep -nF 'return f"raw:link-graph:{index:02d}:{_resource_slug(' support/operator/plan_graph.py`
  - 현재 위치(live 2026-05-30): `support/operator/plan_graph.py:385`  (matches: 1)
  - 근거:
    ```
...
- **Attach raw:agent:NN to step-output observation** — _pointer_
  ...
  - 재파생: `grep -nF 'raw_ref=_raw_ref("agent", index)' support/recording/claims_common.py`
  - 현재 위치(live 2026-06-11, 재파생 재실행): `support/recording/claims_common.py:92`  (matches: 1)
  - 근거:
    ```
...
- **Brick raw record carries raw:brick:NN (raw_ref + raw_refs)** — _pointer_
  ...
  - 재파생: `grep -nF 'def _brick_raw_records(' support/recording/claims_brick.py`
  - 현재 위치(live 2026-06-11, 재파생 재실행): `support/recording/claims_brick.py:26`  (matches: 1)
  - 근거:
    ```
...
- **Link raw record carries raw:link:NN (raw_ref + raw_refs)** — _pointer_
  ...
  - 재파생: `grep -rnF '"raw_ref": _raw_ref("link", index),' support/`
  - 현재 위치(live 2026-05-30): `support/operator/evidence_assembly.py:1548`  (matches: 1)
  - 근거:
    ```
...
- **Stamp capture event with axis-dispatched raw_ref** — _pointer_
  ...
  - 재파생: `grep -nF 'raw_ref=_event_raw_ref(event.event_type, step_index)' support/recording/lifecycle_emit.py`
  - 현재 위치(live 2026-06-11, 재파생 재실행): `support/recording/lifecycle_emit.py:160`  (matches: 1)
  - 근거:
    ```
...
- **Schema-level guarantee: every capture event MUST carry a non-empty raw_ref** — _proof-limit_
  ...
  - 재파생: `grep -nF 'object.__setattr__(self, "raw_ref", _required_text("raw_ref", self.raw_ref))' support/recording/capture.py`
  - 현재 위치(live 2026-05-30): `support/recording/capture.py:166`  (matches: 1)
  - 근거:
    ```
...
- **Persist raw records (each carrying raw_ref) to raw/*.jsonl** — _anchor_
  - 묶는 것: The writer that lands the brick/agent/link raw records (each row embedding its raw:{kind}:NN raw_ref) onto disk at raw/brick-work.jsonl, raw/agent-return.jsonl, raw/link.jsonl under the building root.
  ...
  - 재파생: `grep -nF 'raw_dir / "agent-return.jsonl",' support/recording/raw_claim_trace.py`
  - 현재 위치(live 2026-05-30): `support/recording/raw_claim_trace.py:34`  (matches: 1)
  - 근거:
    ```
...
- **Building-map per-step row producer (wires all axis-crossing refs)** — _anchor_
  ...
  - 재파생: `grep -nF 'input_public_fact_refs=[agent_fact_ref, comparison_ref, movement_ref],' support/recording/building_map_emit.py`
  - 현재 위치(live 2026-06-11, 재파생 재실행): `support/recording/building_map_emit.py:181`  (matches: 1)
  - 근거:
    ```
...
- **agent_binding -> brick_instance_ref wiring (the agent-to-brick crossing)** — _anchor_
  ...
  - 재파생: `grep -nF 'produced_public_fact_refs=[agent_fact_ref],' support/recording/building_map_emit.py`
  - 현재 위치(live 2026-06-11, 재파생 재실행): `support/recording/building_map_emit.py:155`  (matches: 1)
  - 근거:
    ```
...
- **Contract-derived link_edge emitter (LINK backbone field set)** — _derive_
  ...
  - 가리키는 것: building_map_link_edge_specs() in support/recording/contracts.py (the LINK-backbone field-spec), iterated by _build_from_specs which raises if a contract-required crossing field is missing or an undeclared key is supplied.
  - symbol: `operator_evidence.build_link_edge_row`
  - 토큰: `record_label="building_map.link_edge",`
  - 재파생: `grep -nF 'record_label="building_map.link_edge",' support/recording/operator_evidence.py`
  - 현재 위치(live 2026-05-30): `support/recording/operator_evidence.py:242`  (matches: 1)
  - 근거:
    ```
...
- **LINK-axis crossing field contract (single source of the link_edge shape)** — _anchor_
  ...
  - 재파생: `grep -nF 'BUILDING_MAP_LINK_EDGE_REQUIRED_FIELDS: tuple' support/recording/contracts.py`
  - 현재 위치(live 2026-05-30): `support/recording/contracts.py:365`  (matches: 1)
  - 근거:
    ```
...
- **AGENT-axis crossing field contract (agent_binding.brick_instance_ref declared here)** — _anchor_
  ...
  - 재파생: `grep -nF 'BUILDING_MAP_AGENT_BINDING_REQUIRED_FIELDS: tuple' support/recording/contracts.py`
  - 현재 위치(live 2026-05-30): `support/recording/contracts.py:350`  (matches: 1)
  - 근거:
    ```
...
- **Crossing-ref coordinate producer (mints the binding/agent-fact/movement-fact tokens)** — _anchor_
  ...
  - 재파생: `grep -nF 'return f"{kind}:{index:02d}:{slug}"' support/operator/primitives.py`
  - 현재 위치(live 2026-05-30): `support/operator/primitives.py:467`  (matches: 1)
  - 근거:
    ```
...
- **Checker: agent_binding.brick_instance_ref must resolve to a real brick id** — _proof-limit_
  ...
  - 재파생: `grep -nF 'agent binding brick_instance_ref does not resolve' support/checkers/check_building_map_graph.py`
  - 현재 위치(live 2026-05-30): `support/checkers/check_building_map_graph.py:835`  (matches: 1)
  - 근거:
    ```
...
- **Checker: link_edge source/target_brick_instance_ref endpoints must resolve to brick ids** — _proof-limit_
  ...
  - 재파생: `grep -nF 'endpoint must resolve to brick_instance_id, not prefix-only text' support/checkers/check_building_map_graph.py`
  - 현재 위치(live 2026-05-30): `support/checkers/check_building_map_graph.py:338`  (matches: 1)
  - 근거:
    ```
...
- **Checker: link_edge/binding public_fact_refs must resolve through claim_trace** — _proof-limit_
  ...
  - 재파생: `grep -nF 'graph public fact ref does not resolve through claim_trace' support/checkers/check_building_map_graph.py`
  - 현재 위치(live 2026-05-30): `support/checkers/check_building_map_graph.py:568`  (matches: 1)
  - 근거:
    ```
...
- **Closed AgentFact two-field shape** — _anchor_
  ...
  - 재파생: `grep -nF 'Agent records the received work and what was returned.' agent/return_fact.py`
  - 현재 위치(live 2026-05-30): `agent/return_fact.py:118`  (matches: 1)
  - 근거:
    ```
...
- **Concern-kind closed admission set (8 kinds)** — _anchor_
  ...
  - 재파생: `grep -nF 'transition_concern_evidence.concern_kind is not admitted' agent/return_fact.py`
  - 현재 위치(live 2026-05-30): `agent/return_fact.py:100`  (matches: 1)
  - 근거:
    ```
...
- **concern_ref namespace prefix grounding** — _anchor_
  ...
  - 재파생: `grep -nF 'transition_concern_evidence.concern_ref must start with transition-concern:' agent/return_fact.py`
  - 현재 위치(live 2026-05-30): `agent/return_fact.py:97`  (matches: 1)
  - 근거:
    ```
...
- **Concern points back to Brick-boundary coordinates** — _anchor_
  ...
  - 재파생: `grep -nF 'transition_concern_evidence.related_boundary_refs must name Brick boundaries' agent/return_fact.py`
  - 현재 위치(live 2026-05-30): `agent/return_fact.py:112`  (matches: 1)
  - 근거:
    ```
...
- **No-judgment guard: forbidden keys on returned** — _proof-limit_
  ...
  - 가리키는 것: The literal frozenset of 35 forbidden keys (lines 32-68), re-exported and consumed by the recording layer as _ROUTE_REQUEST_FORBIDDEN_KEYS (support/recording/step_outputs.py:11). It is the source-of-truth list a reader checks to see why a judgment field is rejected from returned.
  - symbol: `RETURNED_FORBIDDEN_KEYS`
  - 토큰: `RETURNED_FORBIDDEN_KEYS: frozenset[str] = frozenset(`
  - 재파생: `grep -nF 'RETURNED_FORBIDDEN_KEYS: frozenset[str] = frozenset(' agent/return_fact.py`
  - 현재 위치(live 2026-05-30): `agent/return_fact.py:32`  (matches: 1)
  - 근거:
    ```
...
- **Closed-shape enforcement at record write** — _derive_
  ...
  - 재파생: `grep -nF 'must contain exactly received_work and returned' support/recording/records.py`
  - 현재 위치(live 2026-05-30): `support/recording/records.py:614`  (matches: 1)
  - 근거:
    ```
...
- **Emitter ties received/returned to source refs** — _pointer_
  ...
  - 재파생: `grep -nF '"returned_fact_ref": observation.returned_fact_ref,' support/recording/step_outputs.py`
  - 현재 위치(live 2026-05-30): `support/recording/step_outputs.py:63`  (matches: 1)
  - 근거:
    ```
...
- **Concern is contract-derived FROM returned** — _derive_
  ...
  - 재파생: `grep -nF 'concern = returned.get("transition_concern_evidence")' support/recording/step_outputs.py`
  - 현재 위치(live 2026-05-30): `support/recording/step_outputs.py:294`  (matches: 1)
  - 근거:
    ```
...
- **MovementFact grounding fields (gatefact_reference + transition_history_reference)** — _anchor_
  ...
  - 가리키는 것: gatefact_reference (the GateFact sufficiency verdict that justified the move) and transition_history_reference (the prior transition chain); plus handoff_target_fact (the destination fact). The MovementFact dataclass fields at link/movement.py:33-35 declare these grounding pointers; make_movement_fact at line 42 forwards them without creating any Gate/runtime authority.
  - symbol: `link/movement.py :: MovementFact / make_movement_fact`
  - 토큰: `Build a Link Movement fact without creating Gate or runtime authority.`
  - 재파생: `grep -nF 'Build a Link Movement fact without creating Gate or runtime authority.' link/movement.py`
  - 현재 위치(live 2026-05-30): `link/movement.py:50`  (matches: 1)
  - 근거:
    ```
...
- **per-gate evaluation names the exact BrickComparisonFact coordinates required/missing** — _anchor_
  ...
  - 드리프트 기록(0611 재검증): 옛 anchor는 evaluate_declared_movement_gate가 좌표 목록을 직접 하드코딩했다(BrickComparisonFact.observed_match_kind 포함, 옛 link/gate.py:146). 게이트 시퀀스 개편(739b6d2) 후 좌표 명명은 per-gate 평가(evaluate_declared_gate_ref)로 이동했고, evaluate_declared_movement_gate는 ordered per-gate GateFact들에서 결합 GateFact를 파생한다(observed_match_kind 파라미터는 API 호환용으로만 유지). 개념(좌표 단위 추적가능성)은 동일, 심볼만 이동.
  - symbol: `link/gate.py :: evaluate_declared_gate_ref` (결합: `evaluate_declared_movement_gate` → `derive_movement_gate_fact_from_gate_results`)
  - 토큰: `"BrickComparisonFact.required_return_shape_evidence",`
  - 재파생: `grep -nF '"BrickComparisonFact.required_return_shape_evidence",' link/gate.py`
  - 현재 위치(live 2026-06-11, 재파생 재실행): `link/gate.py:176`  (matches: 1)
  - 근거:
    ```
...
- **Runtime GateFact ties sufficiency to one concrete brick-comparison coordinate (checked_public_fact == evidence_reference)** — _anchor_
  ...
  - symbol: `support/operator/run.py :: _declared_movement_gate_fact`
  - 토큰: `checked = f"brick-comparison:{prepared.building_id}:{prepared.step_rows.step_ref}"`
  - 재파생: `grep -nF 'checked = f"brick-comparison:{prepared.building_id}:{prepared.step_rows.step_ref}"' support/operator/run.py`
  - 현재 위치(live 2026-05-30): `support/operator/run.py:981`  (matches: 1)
  - 근거:
    ```
...
- **Reroute-adoption record carries source_transition_concern_ref (the Agent proposal that justified the reroute)** — _anchor_
  ...
  - symbol: `support/recording/walker_evidence.py :: build_reroute_adoption_record`
  - 토큰: `Emit an ADOPTED reroute-landing record from the contract field-spec.`
  - 재파생: `grep -nF 'Emit an ADOPTED reroute-landing record from the contract field-spec.' support/recording/walker_evidence.py`
  - 현재 위치(live 2026-05-30): `support/recording/walker_evidence.py:94`  (matches: 1)
  - 근거:
    ```
...
- **building-map link_edge contract declares movement_fact_ref + transition_fact_ref as required grounding pointers** — _anchor_
  ...
  - symbol: `support/recording/contracts.py :: BUILDING_MAP_LINK_EDGE_REQUIRED_FIELDS`
  - 토큰: `"movement_fact_ref",`
  - 재파생: `grep -nF '"movement_fact_ref",' support/recording/contracts.py`
  - 현재 위치(live 2026-05-30): `support/recording/contracts.py:372`  (matches: 1)
  - 근거:
    ```
...
- **link_edge emitter wires movement_fact_ref and input_public_fact_refs to the step's concrete fact coordinates** — _anchor_
  ...
  - symbol: `support/operator/evidence_assembly.py :: (building-map assembly) build_link_edge_row call`
  - 토큰: `movement_fact_ref=movement_ref,`
  - 재파생: `grep -nF 'movement_fact_ref=movement_ref,' support/recording/building_map_emit.py`
  - 현재 위치(live 2026-06-11, 재파생 재실행): `support/recording/building_map_emit.py:183`  (matches: 1)
  - 근거:
    ```
...
- **HOLD record grounds a non-adopted reroute to the Agent concern_ref and marks disposition_required** — _anchor_
  ...
  - symbol: `support/operator/walker_hold.py :: _build_hold`
  - 토큰: `source_transition_concern_ref=_optional_text_value(concern.get("concern_ref")) or "",`
  - 재파생: `grep -nF 'source_transition_concern_ref=_optional_text_value(concern.get("concern_ref")) or "",' support/operator/walker_hold.py`
  - 현재 위치(live 2026-06-10, repointed after the dynamic_walker → walker_* decomposition): `support/operator/walker_hold.py:80`  (matches: 1)
  - 근거:
    ```
...
- **caller-supplied MovementFact preserves gatefact_reference + transition_history_reference grounding pointers** — _anchor_
  ...
  - symbol: `support/operator/plan_validation.py :: _caller_link_facts`
  - 토큰: `gatefact_reference=_optional_text_or_none(movement_fact_data.get("gatefact_reference")),`
  - 재파생: `grep -nF 'gatefact_reference=_optional_text_or_none(movement_fact_data.get("gatefact_reference")),' support/operator/plan_validation.py`
  - 현재 위치(live 2026-05-30): `support/operator/plan_validation.py:369`  (matches: 1)
  - 근거:
    ```
...
- **native-dispatch close grounds MovementFact.handoff_target_fact to the next Brick instance and computes the gate verdict** — _anchor_
  ...
  - symbol: `support/operator/building_operation.py :: close_native_dispatch_brick`
  - 토큰: `handoff_target_fact=target_fact,`
  - 재파생: `grep -nF 'handoff_target_fact=target_fact,' support/operator/native_dispatch.py`
  - 현재 위치(live 2026-06-11, 재파생 재실행): `support/operator/native_dispatch.py:722`  (matches: 1)
  - 근거:
    ```
...
- **Central frontier not_proven merge (plan + all steps + failed prep + observation)** — _derive_
  ...
  - symbol: `_adapter_error_frontier_lifecycle_packet (support/operator/evidence_assembly.py)`
  - 토큰: `failed_preparation.not_proven,`
  - 재파생: `grep -nF 'failed_preparation.not_proven,' support/recording/adapter_error_frontier.py`
  - 현재 위치(live 2026-06-11, 재파생 재실행): `support/recording/adapter_error_frontier.py:290`  (matches: 1)
  - 근거:
    ```
...
- **Accumulated multi-step Building result not_proven/proof_limits roll-up** — _derive_
  ...
  - symbol: `run_building_plan (support/operator/run.py)`
  - 토큰: `not_proven=_merge_texts(packet.get("not_proven"), *(r.not_proven for r in step_results)),`
  - 재파생: `grep -nF 'not_proven=_merge_texts(packet.get("not_proven")' support/operator/run.py`
  - 현재 위치(live 2026-05-30): `support/operator/run.py:389`  (matches: 1)
  - 근거:
    ```
...
- **Authority-claim normalization: bare authority words rewritten to explicit 'not proven:' anchors** — _derive_
  ...
  - symbol: `_manifest_not_proven (support/operator/evidence_assembly.py)`
  - 토큰: `if lowered in exact_authority_claims:`
  - 재파생: `grep -nF 'if lowered in exact_authority_claims:' support/recording/claims_common.py`
  - 현재 위치(live 2026-06-11, 재파생 재실행): `support/recording/claims_common.py:67`  (matches: 1)
  - 근거:
    ```
...
- **Dynamic-walker HOLD/reroute not_proven anchor source constant** — _anchor_
  ...
  - symbol: `NOT_PROVEN module constant (support/operator/walker_common.py)`
  - 토큰: `semantic correctness of the agent-proposed reroute`
  - 재파생: `grep -nF 'semantic correctness of the agent-proposed reroute' support/operator/walker_common.py`
  - 현재 위치(live 2026-06-10, repointed after the dynamic_walker → walker_* decomposition): `support/operator/walker_common.py:28`  (matches: 1)
  - 근거:
    ```
...
- **Axis-specific transition_concern not_proven derivation** — _derive_
  ...
  - symbol: `write_step_outputs (support/recording/step_outputs.py)`
  - 토큰: `transition concern semantic correctness`
  - 재파생: `grep -nF 'transition concern semantic correctness' support/recording/step_outputs.py`
  - 현재 위치(live 2026-05-30): `support/recording/step_outputs.py:160`  (matches: 1)
  - 근거:
    ```
                    _merge_texts(
...
- **Canonical default proof_limits source-of-truth tuple** — _anchor_
  ...
  - symbol: `DEFAULT_PROOF_LIMITS module constant (support/recording/capture.py)`
  - 토큰: `DEFAULT_PROOF_LIMITS = (`
  - 재파생: `grep -nF 'DEFAULT_PROOF_LIMITS = (' support/recording/capture.py`
  - 현재 위치(live 2026-05-30): `support/recording/capture.py:118`  (matches: 1)
  - 근거:
    ```
...
- **proof_limits normalizer with conservative default fallback** — _derive_
  ...
  - symbol: `_proof_limits_tuple (support/operator/primitives.py)`
  - 토큰: `return tuple(facts) or _DEFAULT_PROOF_LIMITS`
  - 재파생: `grep -nF 'return tuple(facts) or _DEFAULT_PROOF_LIMITS' support/operator/primitives.py`
  - 현재 위치(live 2026-05-30): `support/operator/primitives.py:531`  (matches: 1)
  - 근거:
    ```
        if values is None:
...
- **Contract = single canonical evidence-shape source (the search_for() equivalent)** — _anchor_
  ...
  - symbol: `module docstring block in support/recording/contracts.py (DYNAMIC-WALKER EVIDENCE-SHAPE CONTRACT section)`
  - 토큰: `only a change to`
  - 재파생: `grep -nF 'only a change to' support/recording/contracts.py`
  - 현재 위치(live 2026-05-30): `support/recording/contracts.py:88`  (matches: 1)
  - 근거:
    ```
...
- **Per-axis field-spec deriver — capture-event shape computed from declared header+payload, not stored** — _derive_
  ...
  - 재파생: `grep -nF 'def capture_event_field_specs' support/recording/contracts.py`
  - 현재 위치(live 2026-05-30): `support/recording/contracts.py:318`  (matches: 1)
  - 근거:
    ```
...
- **Axis attribution declared ONCE in contract, with an explicit anti-tautology warning** — _anchor_
  ...
  - 재파생: `grep -nF 'Declared axis_attribution per event_type' support/recording/contracts.py`
  - 현재 위치(live 2026-05-30): `support/recording/contracts.py:255`  (matches: 1)
  - 근거:
    ```
...
- **Frontier observation proof-limit literals fixed in contract (provenance disclaimer attached at source)** — _proof-limit_
  ...
  - 재파생: `grep -nF 'graph support projection only' support/recording/contracts.py`
  - 현재 위치(live 2026-05-30): `support/recording/contracts.py:412`  (matches: 1)
  - 근거:
    ```
...
- **Emitter iterates the contract spec — drift-proof builder (operator evidence: capture events + building-map rows + frontier)** — _pointer_
  ...
  - 가리키는 것: The contract specs (capture_event_field_specs / building_map_*_specs / frontier_observation_specs imported at the top of this file). A missing required field raises ('contract-required field ... was not supplied'); an extra key raises here — so the emitted record is provably tied to the contract source and cannot drift. Replaces the formerly-inline literals in support/operator/evidence_assembly.py.
  - symbol: `operator_evidence._build_from_specs`
  - 토큰: `values carry undeclared field(s) not in the`
  - 재파생: `grep -nF 'values carry undeclared field(s) not in the' support/recording/operator_evidence.py`
  - 현재 위치(live 2026-05-30): `support/recording/operator_evidence.py:64`  (matches: 1)
  - 근거:
    ```
            raise ValueError(
...
- **Second emitter (dynamic-walker) iterates the SAME contract — reroute-adoption / HOLD / structured-field records** — _pointer_
  ...
  - 토큰: `values carry undeclared field(s) not in the`
  - 재파생: `grep -nF 'values carry undeclared field(s) not in the' support/recording/walker_evidence.py`
  - 현재 위치(live 2026-05-30): `support/recording/walker_evidence.py:54`  (matches: 1)
  - 근거:
    ```
            raise ValueError(
                f"{record_label}: values carry undeclared field(s) not in the "
...
- **ζ6 oracle re-derives the shape from the SAME contract and rejects emitter drift (DROPPED / ADDED field)** — _proof-limit_
  ...
  - 재파생: `grep -nF 'emitter ADDED undeclared field(s) not in contract' support/checkers/check_recording_checker_derived_contract.py`
  - 현재 위치(live 2026-05-30): `support/checkers/check_recording_checker_derived_contract.py:125`  (matches: 1)
  - 근거:
    ```
...
- **Anti-tautology independent axis rule — checker derives expected axis from event_type NAME, not the contract dict** — _proof-limit_
  ...
  - 재파생: `grep -nF 'def _independent_expected_axis' support/checkers/check_recording_checker_derived_contract.py`
  - 현재 위치(live 2026-05-30): `support/checkers/check_recording_checker_derived_contract.py:82`  (matches: 1)
  - 근거:
    ```
...
- **plan_snapshot content-hash anchor** — _anchor_
  ...
  - 가리키는 것: The Brick-owned source plan under brick/building_plans/: the sha256 (plan_hash) is taken over the exact canonical sorted-key JSON string (plan_rows_copy), so a reviewer can re-derive the hash from the recorded copy and confirm exactly which declared plan produced the evidence
  - symbol: `_plan_snapshot`
  - 토큰: `plan_hash_basis`
  - 재파생: `grep -nF '"plan_hash_basis": "canonical sorted-key JSON of the declared plan body",' support/recording/declaration_packets.py`
  - 현재 위치(live 2026-06-11, 재파생 재실행): `support/recording/declaration_packets.py:973`  (matches: 1)
  - 근거:
    ```
...
- **step-output observation ref binding** — _pointer_
  ...
  - 재파생: `grep -nF 'received_work_ref=_step_fact_ref("brick-work", index, step_ref)' support/recording/claims_common.py`
  - 현재 위치(live 2026-06-11, 재파생 재실행): `support/recording/claims_common.py:90`  (matches: 1)
  - 근거:
    ```
...
- **central claim-fact ref->raw binding** — _anchor_
  ...
  - 재파생: `grep -nF '"fact_ref": fact_ref,' support/recording/claims_common.py`
  - 현재 위치(live 2026-06-11, 재파생 재실행): `support/recording/claims_common.py:49`  (matches: 1)
  - 근거:
    ```
    return {
            "axis": axis,
...
- **raw manifest path->source attribution** — _anchor_
  ...
  - 가리키는 것: Each on-disk raw file path is tied to its producing source string (support/operator/run.py run_building_plan), its axis_owner (Brick/Agent/Link), and the per-step raw_refs (raw:brick:NN ...) it contains -- so a reader can trace any raw stream back to who wrote it and which step refs it covers
  - symbol: `_accumulated_raw_manifest`
  - 토큰: `"source": "support/operator/run.py run_building_plan declared Brick rows",`
  - 재파생: `grep -nF '"source": "support/operator/run.py run_building_plan declared Brick rows",' support/recording/lifecycle_emit.py`
  - 현재 위치(live 2026-06-11, 재파생 재실행): `support/recording/lifecycle_emit.py:189`  (matches: 1)
  - 근거:
    ```
...
- **movement claim source attribution** — _derive_
  ...
  - 재파생: `grep -nF '"movement_source": "caller-declared Building Plan Link row",' support/recording/claims_link.py`
  - 현재 위치(live 2026-06-11, 재파생 재실행): `support/recording/claims_link.py:152`  (matches: 2; also :607)
  - 근거:
    ```
    fact_body = {
                "movement": movement,
...
- **movement supporting-evidence ref bundle** — _derive_
  ...
  - 재파생: `grep -nF 'refs.extend(_gate_claim_fact_refs(result.completion.crossing_record, index, step_ref))' support/recording/claims_link.py`
  - 현재 위치(live 2026-06-11, 재파생 재실행): `support/recording/claims_link.py:573`  (matches: 1)
  - 근거:
    ```
    refs = [
...
- **brick comparison observation anchor** — _derive_
  ...
  - 재파생: `grep -nF 'fact_ref=_step_fact_ref("brick-comparison", index, prepared.step_rows.step_ref),' support/recording/claims_brick.py`
  - 현재 위치(live 2026-06-11, 재파생 재실행): `support/recording/claims_brick.py:76`  (matches: 1)
  - 근거:
    ```
...
- **buildings_root_for — project_ref → 증거 동네 단일 파생 seam** — _anchor_
  ...
  - symbol: `support/recording/capture.py :: buildings_root_for`
  - 토큰: `return REPO_ROOT / "project" / project_id / "buildings"`
  - 재파생: `grep -nF 'return REPO_ROOT / "project" / project_id / "buildings"' support/recording/capture.py`
  - 현재 위치(live 2026-06-11): `support/recording/capture.py:69`  (matches: 1)
  - 근거:
    ```
