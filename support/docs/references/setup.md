...
A green gate is **exit code 0**. On success the runner prints one `profile passed:` line per profile (12 profiles live under `support/checkers/profiles/`), then the closing proof-limit line. On any rejection it prints `profile runner rejected evidence: ...` to stderr and returns exit code 1.
