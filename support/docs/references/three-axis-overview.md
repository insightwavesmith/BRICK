...
`support/` is not an axis. It contains runner, adapter, recording, projection, and checker mechanics. Those mechanics can validate shape, connect an admitted adapter, write evidence, and render projections, but they do not own Brick / Agent / Link meaning.
...
Vessels are created by the creation verb (`support/operator/project_creation.py`, surfaced as the `project-creation` skill). A hand-assembled vessel missing the charter or the declaration is checker-RED.
