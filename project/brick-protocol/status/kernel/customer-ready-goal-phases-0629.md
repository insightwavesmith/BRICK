...
PASS: origin/main clone → install → init/doctor/auth/onboard → build/fire → evidence/frontier inspection on a clean machine, **documented steps ONLY**, `frontier=complete`, ZERO undocumented manual steps, NO hidden machine-local dep. P5-B4 fresh-HOME profile evidence removes the former empty-HOME `intake_evidence_projection_case` item as an unmeasured active blocker; it does not replace the full fresh-clone P7 proof.
