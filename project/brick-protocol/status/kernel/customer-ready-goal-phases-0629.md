...
- **남은 임계경로:** P5(#3/#4/#6 + optional #5 strict-removal review) → P7(fresh-machine) → P8(dogfood 골) → customer release pruning → FINAL architecture cleanup.
...
**STATUS (0630 after P5-B1):** 정직성 골격 OK · 게이트2(brick_cli_entrypoint·building_operator_driver0) green. **#1 output_root hardcode는 P3에서 CLOSED. #2 [HIGH] README/quickstart preset-adapter mismatch는 `bf67279`에서 CLOSED. #5 [MED] launch-guide/Python helper drift는 customer-first-run 표면에서 advanced/internal로 fenced/cleaned; strict snippet deletion이 필요하면 follow-up.** 남은 GAPS: **#3 [MED]** first-run frontier 스토리 경로별 불일치(build=complete/onboard=incomplete) · **#4 [MED·Agent]** first-use adapter population NOT_PROVEN(소유표면 미정) · **#6 [LOW→P7]** fresh-machine 미증명. 다음 P5 빌딩 후보 = #3 frontier story + #4 FIRST_USE adapter population scope, 또는 #5 strict-removal review if Smith requires zero Python snippets.
