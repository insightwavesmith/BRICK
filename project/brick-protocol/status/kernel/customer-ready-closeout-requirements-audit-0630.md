...
| R1b | Main agent must preserve token-cost discipline: avoid broad raw/evidence/check-log reads, use field extraction + short verdicts, and delegate broad investigations to Codex subagents when useful. | `customer-ready-closeout-goal-0630.md` 0d added this turn. Hook/checker enforceability under investigation; discipline is binding immediately as prompt/goal rule. | binding requirement added; hook feasibility not yet proven |
