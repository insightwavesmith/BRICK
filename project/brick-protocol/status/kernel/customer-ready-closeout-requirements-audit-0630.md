...
| R1a | Main agent must actually draw and fire Buildings for G1/G2/G3 implementation slices so the main-agent building-running skill chain is exercised end to end. | `customer-ready-closeout-goal-0630.md` section 0c added by `ec380ca`; direct provider_preflight patch was reverted before commit. | binding requirement added; live compliance not yet proven until next implementation slice is fired as a Building |
