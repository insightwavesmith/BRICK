...
**"개발에서 코덱스는 이제 제외한다. 오푸스 4.8 xhigh + fugu 둘로 가자. 코덱스는 쉬운 것도 못 한다. 지금 빌딩까지만 굴린다."** 적용: 신규 발주의 시공(work)·수리 레인 = **opus-4.8 xhigh(단순~중간) / fugu(복잡·얽힘·엔진급)** 2트랙. codex는 현재 걷는 빌딩을 끝으로 dev 레인에서 퇴역. 신규 발주의 code-attack 렌즈·closure도 opus/gemini/fugu로 대체(카탈로그상 codex 잔존 좌표는 초안기 티어-해석 이관 시 일괄 정리 — 행 23과 합류). 초안기 캐스팅 상수의 codex 행들은 티어-해석 수리 전까지 발주 시 운영자 재캐스팅으로 무력화.
