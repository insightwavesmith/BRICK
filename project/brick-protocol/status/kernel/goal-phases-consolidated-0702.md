...
| **P-직결 트랙(주차 — Smith 동행 방아쇠)** | 홀드-직결 확장 묶음: ①초안기의 expansion_budget 자동 기입(2단-의도 감지 시) ②직결 라이브 1회 실증(판정 ④, 검증기 감시+새-선언 폴백) ③바둑 반복(개정 2호+) ④재개-시점 캐스팅 오버레이(행 24, "개구리다→fable5 교체 이어가기"). 0707 실측 기반: 기계 전부 실존, 예산 전제에서 1회 반려 — 현 웨이브 정리 후 Smith와 함께 첫 수 | **주차(같이 가자)** |
