...
정본 = postmortem-deep-0706a-diagnosis-synthesis.md. 순서 판단(COO): 병렬 = R1(parents[N] 정체성 가드+레지스트리 체커) · R4(어댑터 오류 분류+타임스탬프 보존) · R5+R8(부재주장 도메인 라벨 의무+거짓 'git 없음' 자기제한 제거 — 규율 계열 합본). **직렬(cpath 수술 뒤, walker_kernel 충돌)** = R2(carry 공용헬퍼+전채널 체커+remint 5좌표 감사 — 푸구 실측 갭 포함) · R3(팬-이음새 driver-사망 증거 포착+수신-후-무응답 loud HOLD, Case 9 선행). 문서-측 = R6(처방-착지 추적) · R7(사건 처분 단일소스 = 마스터 큐 선언). 별도 소형 = R9(모델-레인 규율 정합화: discipline:model-lane-matching 선언을 0705/0706 실정책과 일치시키기 — 부검이 표면화한 선언-정책 충돌).
