...
- **푸구 사인 정정(실측 확정)**: 도구 제거 -c 수리는 0624 기랜딩(e4466b2b)이었고, 실사인 = **Sakana API가 walker 발주 packet을 HTTP 451(content policy)로 차단** — 작은/무해/66KB 프롬프트 통과, 19KB 실packet만 재현(크기 아닌 내용·조합). 어댑터 스택 자체는 pong 실반환으로 green. 푸구 캐스팅 보류 유지, Sakana 정책 조사 서브에이전트(sonnet) 진행 중, 슬롯(2→1 전환분) 되돌리기는 Smith 판단 대기.
