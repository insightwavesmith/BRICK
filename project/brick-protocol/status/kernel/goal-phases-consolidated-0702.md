...
### C. 대형·스코프 결정 (Smith 프레이밍 권장)
- **A+ W1 Contract Kernel**: 착수 게이트 4/4 개방됨. replay 수리는 W1 비의존. 다슬라이스 아키텍처 웨이브 — 밤중 단독 착수 vs Smith 프레이밍 대기 판단 필요.
- **A+ W2 (mutation-RED manifest 게이트) — 0706 회고로 명시 목표 격상**: 공허 검증 차단의 기계 집행("신규 표면은 RED 명세 없이 반입 불가"). 근거 = erg1 실수 6호(변이-RED 육안 오독으로 공허 변이를 통과 처리 — COO가 자가 적발했지만 못 잡을 수도 있었다). 이건 "AI가 자기 검증을 자기도 못 믿는 구간"의 제거라 흩어진 후보가 아니라 명시 목표다.
...
- ~~t7b replay 다중화~~ → **Smith 0706 승인(D1)** — 전용 설계 슬라이스 발주됨(t7b-replaydesign-0706a, read-only: 정합 미러링 지점 전수 판독 + A/B 확정 + RED→GREEN 게이트 사양 + 시공계획 판정. design·공격렌즈=fable5 xhigh 위험-비례 승격, 시공 자체는 여전히 Smith 게이트 밖). **격상(0706 회고)**: 선택 백로그가 아니라 **다단(2-개정+) 홀드 운영의 전제 조건** — 없으면 T10류 다단 운전이 매번 fresh 재발주로 귀결(0705 봉쇄 5기 실측 비용: 판단·산출 다 있는데 완주 도장만 못 찍음).
