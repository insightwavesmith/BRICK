...
**★decl 소실 위험(0706 저녁 실측·정직 고지)**: decl 3개는 `/private/tmp/*-0706e-decl.json`에 있음 = 시스템 temp, 재부팅/세션전환 시 **소실 가능**. repo 안으로 옮기려 했으나 `pending-launches-0706e/`가 package_path_admission 세트 밖이라 스윕 RED → 걷어냄(admission 등록은 별도 작업). **다음 세션 첫 확인: `ls /private/tmp/*-0706e-decl.json` — 있으면 그대로 발사, 없으면 아래 §각 빌딩 명세(목적·시공지점·write_scope·Deliverables·검증선 전부 기재됨)로 재작성**(brick-task-author 스킬 + 이 문서). 재작성은 정보 완비라 어렵지 않음. fugupacket의 D3 아티팩트(fugu-failing-prompt.json 19KB·fugu-bisect.sh)도 /private/tmp — 소실 시 fugu-bisect.sh는 이 문서 §푸구 근본원인의 재현명령으로, failing-prompt는 walker 발주 packet 캡처로 재생성.
