...
## T7. 실패 복구 경로 — resume·홀드·WIP 4결함 (✅ 전체 종결 — 재발주 금지, 이력 보존용)

> 0704 종결 요약: S-a/S-b/S-c/S-d 웨이브 A 랜딩. 결함① stale(브리지 실증,
> resume-raise-budget-bridge-0704.md) · 결함② stale 3중 확인 · 결함③ 정정경로 문서
> 랜딩 · 결함④ = 0704 밤 Smith 확정 "설계 예외, 수리 없음"(+ 증명-예산 앵커도 수리
> 불요 — 기전: diff 없음이라 보존 대상 자체가 없음, 원장·manifest로 수동 복구 가능).

**실측/현재 기전**:
- 결함① raise 예산 미반영: 처분의 budget_increment는 support가 읽어 budget_delta로 엔진에 넘긴다(walker_resume.py:265-272, ResumeSeed로 :343-359). 총량 반영은 walker_kernel.py 내부 — ~~미확인~~ **(0704 종결: T7-Sa 브리지 실증으로 확인 완료, 결함 부존재 — 이 문단의 "미확인"은 이력 보존)**.
...
## T8. 의사결정자 증거 투영 — 인간 게이트 처리량 (S-a ✅ 웨이브 A 랜딩 — 잔여 S-b만)
...
## T9. 체커-동반 개발 원칙 (구 "이식성" — Smith 0704 재정의) (S-a ✅ 웨이브 A 랜딩)
...
## T11. 교훈의 구조화 축적 — 실측 원장 (✅ 전 슬라이스 웨이브 A 랜딩 — 재발주 금지)
