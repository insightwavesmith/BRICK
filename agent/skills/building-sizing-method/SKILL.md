...
> 손-작성 `graph_packet` JSON(`brick build --graph <packet>`)은 폐기 방향이고, 지금은 DSL이 아직
> 못 하는 두 가지(`sibling_independence`, 노드별 explicit `write_scope` 좁히기)의 low-level escape
> hatch로만 남는다. 이 스킬은 발사하지 않는다.
...
`compose_building()`, Rule 9)로 들어가는 두 입력 모드다. `build()`, `fan()`, `assemble()`,
...
폐기 방향이다 — DSL이 아직 `sibling_independence`와 노드별 explicit `write_scope` 좁히기를 지원하지
않는 동안만 low-level escape hatch로 남는다. 공식 route처럼 일반 안내하지 마라.
...
(손-작성 `graph_packet` JSON 경로는 폐기 방향이라 DSL이 못 하는 두 경우의 escape hatch로만 쓴다.)
