...
own `Agent` tool), not through `support/operator/run.py`'s Building Plan walk.
...
`support/operator/building_operation.py` (re-exported from
...
handle = open_native_dispatch_brick(
    ...
    # later, when brick/work.py parse_required_return_shape is called at the gate
...
(`support/onboarding/claude-hooks/`, `support/onboarding/codex-hooks/`) into
...
- OPT-IN ACTIVATION: the hooks do nothing until the onboard recording step
  (`support/operator/onboard.py --recording`) — or an equivalent manual merge —
...
```text
Brick seam (support/operator/building_operation.py)  = the engine API.
