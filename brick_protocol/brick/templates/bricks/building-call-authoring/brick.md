...
3. STEP3 structure: produce `structure_draft` with Brick-plane nodes, Link-shaped edges, gate_state, and held_for_coo_review lifecycle evidence. Agent-plane content is limited to role_need, capability_need, and write_need. If graph structure is drafted, place `structure_plan_draft` inside `structure_draft` using only `nodes`, `edges`, `coo_gate_edge`, `fan_out_groups`, `fan_in_groups`, `reroute_budgets` / `node_reroute_budgets`, and `terminal`; `fan_out_groups` require `coo_gate_edge.state: held_for_coo_review` before any later lowering, with COO confirmation of N at that hold.
