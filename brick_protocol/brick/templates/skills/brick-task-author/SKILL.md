...
(`brick_protocol/support/operator/assembly.py`) plus `run_building_plan()`이다. graph packet JSON을
...
1. **direct quick_check** (`direct_quick_check`) — 읽기 전용 확인. Worked example:
   "문서 A와 체크커 핀 B가 같은 라벨을 갖는지 확인하고, diff 없이 관찰만 반환하라."
   direct_preset은 admission + fast_confirm 뒤에만 가능하며 launch authorization이 아니다.
2. **direct quick_fix** (`direct_quick_fix`) — 작고 낮은 위험의 한 지점 수리. Worked example:
...
- **예산 주입(0702 실전 검증)**: `run_approve_entry(ROOT, action="raise", author_ref="coo:smith", budget_increment=N, adapter_cwd=<격리경로>)` — "예산 미선언" 홀드는 `budget_exhausted`로 분류돼 raise 대상. concern 접수·완주는 `action="forward"`. 기본 예산 5는 brick_protocol/brick/templates/reroute-defaults.yaml(Smith 선언) — compact DSL 자동 적용은 인체공학 4번 대기.
