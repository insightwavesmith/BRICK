---
name: make-a-gate
description: BRICK 새 게이트(link-gate) 만들기. Link 게이트 어휘를 1개 추가할 때 — brick_protocol/link/spec.py GATE_REGISTRY에 행 APPEND + 병렬 표면 동기화. 끝에 레지스트리 로드 + 카탈로그 크로스체크 green = placement 가능 증명. 단일소스 가드(check_gate_registry_single_source) 준수.
---

# 새 게이트 만들기 (single-source APPEND → sync → 사용 가능 확인)

법: 게이트 어휘는 **`brick_protocol/link/spec.py` `GATE_REGISTRY` 한 곳**이 단일소스(STRUCT-SURGERY ② landed).
모든 파생(`brick_protocol/link/gate.py`의 `DECLARED_GATE_REFS`, HUMAN/AUTO 분할, `_GATE_REQUIRED_RETURN_FIELDS`,
...
## 1단계 — APPEND 1행 (`brick_protocol/link/spec.py` `GATE_REGISTRY`)
...
```
이것만으로 `brick_protocol/link/gate.py`의 `DECLARED_GATE_REFS`/HUMAN·AUTO 분할/`_GATE_REQUIRED_RETURN_FIELDS`,
...
1. **`brick_protocol/support/operator/building_operation_common.py`** — `COMPACT_LINK_GATE_TOKENS`는
   ...
   **정확히** 요구 → **`[4]` 항목을 추가**해야 한다 (concept_token 있는 게이트면).
   ...
   ```
2. **`brick_protocol/link/gate.yaml` `declared_gate_refs`** — `{gate_ref: link-gate:<name>, meaning: …}` 추가
   (레지스트리와 drift나면 RED).
3. **프리셋서 부를 수 있게 하려면** `brick_protocol/link/gate.yaml` `post_d_base_gate_matrix.concepts`에
...
- **RULE 1** — `brick_protocol/link/spec.py` 밖에서 `link-gate:` 문자열 **2개 이상**을 담은 리터럴 금지(어휘
  재-열거 금지). 단일 비교(`x == "link-gate:human"`)나 1-원소 값은 OK.
- **RULE 2** — support materializer서 placement 규칙을 **다시 짓지 마라** — `gate_placement_for_row`에
  위임. (스킬 콘텐츠인 이 .md는 brick_protocol/agent/brick/link/support 트리 밖이라 RULE 영향 없음.)
...
HOME=$(mktemp -d) PYTHONPATH=brick_protocol/support/import_identity uv run python3 \
  brick_protocol/support/checkers/check_profile.py --profile gate_registry_single_source
HOME=$(mktemp -d) PYTHONPATH=brick_protocol/support/import_identity uv run python3 \
  brick_protocol/support/checkers/check_brick_template_catalog_restructure.py    # link_gate_token_drift 통과
```
