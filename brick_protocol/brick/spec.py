...
(``brick_protocol/brick/work.py`` ``BrickWork`` + ``parse_required_return_shape``); this module
...
support files (one of them named for a DIFFERENT axis). It owns:
  ...
  * ``BRICK_ROW_ALLOWED_KEYS`` — the admitted Brick-row key-set a plan may carry
    (moved from ``brick_protocol/support/operator/primitives.py`` at E2/S9).
  * ``WriteScope`` value object — the Brick-declared write envelope SHAPE
    (``allowed_paths`` / ``forbidden_paths``) + its ``clean`` / ``validate``
    discipline (moved from ``brick_protocol/support/connection/agent_adapter.py`` at E2/S9 — a
  ...
  * ``DERIVED_WORKTREE_WRITE_SCOPE`` — the default worktree write envelope (moved
    from ``brick_protocol/support/operator/assembly.py`` at E2/S9).
  * the write-NEED value interpreter (``declared_brick_write_need`` /
    ...
    ``brick_protocol/support/operator/plan_validation.py`` at E2/S9).
...
# brick_protocol/brick/work.py + brick_protocol/brick/comparison.py.
...
# casting kwarg map + bag builder from ``brick_protocol/agent/spec.py`` (brick -> agent is acyclic;
# brick_protocol/agent/spec.py imports nothing from brick). These are re-imports, not re-definitions.
...
# carry. Moved from brick_protocol/support/operator/primitives.py: the set of keys a builder is
...
# Moved from brick_protocol/support/operator/assembly.py; byte-identical to the prior literal.
...
# brick_protocol/support/connection/agent_adapter.py (a support file named for the AGENT axis):
...
class WriteScope:
    ...
    def _reject_bare_dir_write_path(label: str, value: str) -> None:
        ...
        ``brick_protocol/brick/comparison.py:_path_matches_scope`` (the Brick-axis written-vs-scope
        ...
        no glob char (e.g. ``"brick_protocol/support/"``) therefore matches ONLY the literal
        ...
        Exact-file entries (``AGENTS.md``, ``brick_protocol/brick/work.py``) and glob entries
        (``brick_protocol/support/*``, ``brick_protocol/support/**``) are unaffected.
        """
...
# brick_protocol/support/operator/plan_validation.py. Support re-imports these names (a re-import
...
# ---------------------------------------------------------------------------
# BRICK AUTHORING CONTRACT (E2/S1, moved from brick_protocol/support/operator/assembly.py). The
...
# property and is imported from ``brick_protocol/agent/spec.py`` (``_build_casting_bag`` /
...
# E2/S1 plan they STAY in ``brick_protocol/support/operator/assembly.py`` (the graph wiring still
