...
→ brick_protocol/support/operator/run.py walker
→ active vessel evidence root
→ reporter / Slack / frontier
```

현재 public authoring funnel도 이 build surface 안에서 하나다. task가 있으면 caller/COO가
`--preset`, 또는 `--building-case` + `--intensity`, 또는 확인된 `--building-call` JSON 중
하나를 명시한다. 명시가 없으면 fail-closed하며 write-capable 여부로 fast-fix를 고르지 않는다.
`quick_check`/`quick_fix`만 `--direct-preset --fast-confirm` 뒤 직접 lowering한다. 그 외 easy와
normal/complex/critical은 `building-chain-preset:building-call-authoring`을 먼저 걸어 발주 초안을
만들고, COO confirm 뒤 canonical lowering한다.

기존 표현인 “프리셋 모드와 DSL 그래프 모드는 **같은 공식 build surface로 들어가는 두 입력 모드**”는
엔진 수렴 설명으로만 남긴다. `assemble()` / `build()` / `fan()`과 `run_building_plan()`은 이제
Builder 내부 재료다. `--graph-decl`은 기존 canonical graph 선언의 호환 transport이고 신규 고객
입구가 아니다. graph packet JSON을 `brick build --graph <packet>` 또는
`support.operator.cli build --graph <packet>`에 넘기던 “같은 공식 route의 저수준 입력”은 retired다.
Profile compatibility note: `brick_protocol.support.operator.cli build --graph <packet>`, `brick build --graph <packet.json>` /
...
│
├─ 검토한 프리셋이 이미 있음
│     → `--preset` 명시. render_preset_ranking_packet는 advisory일 뿐 자동선택하지 않음.
├─ quick_check / quick_fix이고 fast confirm 가능
│     → `--building-case ... --intensity easy --direct-preset --fast-confirm`
└─ 그 외 easy / normal / complex / critical
      → `--building-case ... --intensity ...`
      → building-call-authoring Building → COO confirm → confirmed JSON → canonical lowering
```
...
Plan → `brick_protocol/support/operator/run.py` walker → active vessel evidence root → reporter/Slack/frontier로
...
brick_protocol/support/model/checker/Slack은 source truth나 quality/success judge가 아니다.
...
<이 task를 쓰기 전 직접 grep/read한 파일을 file:line까지 하나씩 나열. 산문에 묻지 마라 —
 ...
 실측: returns-persistence가 brick_protocol/support/connection/만 grep하고 brick_protocol/support/recording/의 진짜
...
<read_scope는 대개 Required Sources+Context가 지명한 범위. write_scope는 glob 전수
 열거("brick_protocol/support/operator/**" 식, ★"brick_protocol/support/" 단독 금지 = fnmatch 함정). Required Sources에
 ...
 **레인-불가능 수용 기준은 레인 D로 배정 금지(0703 #14 실측)**: 라이브 전용 상태(언트래킹
 잔해)·write_scope 밖 수리가 필요한 기준은 재파견을 아무리 돌려도 레인이 못 채운다 — 자동
 반려 라우트는 ①선언된 proof_obligations rc 불일치에만 발화(산문 계약은 기계 게이트 없음)
 ②격리 워크트리+scope 안에서 고칠 수 있는 것만 해결한다. 그런 기준은 계약에 "COO 게이트
 항목"으로 명시 분리하고, 레인엔 실행가능한 조각(격리 green + 원인 격리 증명)만 맡겨라.
 기계 반려 사이클을 원하는 D는 산문이 아니라 proof_obligations= 로 선언하라.
...
템플릿이 아니라 `task_intake` 스킬 + `brick_protocol/brick/templates/tasks/source-template.md`(Deep
...
## Building Call v1.1 Quick Path

Building Call 요청은 먼저 아래 네 라벨 중 하나로만 분류한다. 이 절은 빠른 authoring
판단용이다. launch authorization, success/quality judgment, Movement authority,
route_target authority, adapter/model/provider/selected_* 노출을 만들지 않는다. direct_preset
escape hatch는 `direct quick_check`와 `direct quick_fix`에만 있고, 둘 다
direct_preset admission과 `fast_confirm`이 있어야 한다.

1. **direct quick_check** (`quick_check`) — 읽기 전용 확인. Worked example:
   "문서 A와 체크커 핀 B가 같은 라벨을 갖는지 확인하고, diff 없이 관찰만 반환하라."
   direct_preset은 admission + fast_confirm 뒤에만 가능하며 launch authorization이 아니다.
2. **direct quick_fix** (`quick_fix`) — 작고 낮은 위험의 한 지점 수리. Worked example:
   "이미 정해진 문서 한 단락과 그 focused profile pin만 같은 문구로 맞춰라." direct_preset은
   admission + fast_confirm 뒤에만 가능하며 launch authorization이 아니다.
3. **order_authoring** (`order_authoring`) — 표준 작업, 여러 파일, 불명확한 범위, 또는
   quick_*로 충분하지 않은 요청. Worked example: "STEP1-STEP5 Building Call draft를 작성하고
   remaining_delta에 lowering 전 필요한 결정을 남겨라."
4. **human_gate_first / forbidden direct** (`human_gate_first`) — credential/secret,
   destructive/irreversible, high-impact boundary, 또는 direct_preset 금지 요청. Worked example:
   "금지 경로 수정이나 외부 효과가 섞이면 direct quick_*가 아니라 human gate 필요 상태로
   둔다."
...
# 토큰겹침 점수 desc. ADVISORY(자동선택 아님) — 내가 골라 chain_preset_ref로 박는다. 카탈로그=brick_protocol/brick/templates/presets/
```
공식 입력은 CLI build다. 예:
...
# quick direct lowering: 네 선언이 모두 있어야 한다
brick build --task "$TASK" --building-case quick_fix --intensity easy --direct-preset --fast-confirm --real-provider

# normal/complex/critical: draft-only authoring Building을 먼저 실행한다
brick build --task "$TASK" --building-case standard_delivery --intensity normal --real-provider

# COO가 검토·확인한 Building Call JSON을 canonical lowerer로 보낸다
brick build --building-call /path/to/confirmed-building-call.json --real-provider
```
...
이 제목 아래 DSL은 Builder 내부 graph/materialization 재료를 설명하는 호환 절이다. 현재 신규
public authoring은 위 Building Call funnel을 쓴다. 완성된 기존 graph 선언을 운반해야 할 때만
`brick build --graph-decl <decl>` 호환 transport를 사용하며, 별도 실행 입구로 가르치지 않는다.
...
**fan() 위치 3법칙** (`brick_protocol/support/operator/assembly.py` build() 하강부 실측):
...
**단, 불가침은 산문만으로 끝내지 마라(0703 감사 교정 — Brick FixB)**: Hard constraints
산문에 "수정 금지/불가침"으로 적는 파일·글로브는 **반드시 `write_scope.forbidden_paths`에도
문자 그대로 미러**한다. "넓게 vs 정밀"은 소프트로 남지만, **불가침만은 하드 벽**이다 —
0703 감사 실측: 산문 펜스 12개 빌딩 전부 기계 보호 0(정상 편집 하나만 섞이면 금지경로
위반이 무제한 통과, 위반방향 게이트는 Phase 2 빌딩으로 배선 중). 게이트가 랜딩되기
전에도 미러는 의무다(랜딩 즉시 소급 보호를 받는다).
...
| `ModuleNotFoundError: No module named 'support'` | `PYTHONPATH=brick_protocol/support/import_identity:.` — repo 루트(`:.`) 누락이 원인, cd만으론 부족 |
...
4. 쓰기 노드 = 노드 `write=True` + 발사 `write_scope` **둘 다** (하나만이면 read-only smoke). glob은 `brick_protocol/support/operator/**` 꼴 (★`brick_protocol/support/` 금지 = fnmatch 함정).
5. `adapter_timeout_seconds` 상향 — one-call build() 기본 120초는 정독/구현 레인에 짧다. 공식 continuation은 `brick resume --decl <JSON>`만 사용한다. resume 작업장은 driver가 검증된 WIP ref/기존 엔진 워크트리에서 자동 재수화하고 close한다.
...
- 워크트리에서 `PYTHONPATH=brick_protocol/support/import_identity:. python3 brick_protocol/support/checkers/check_profile.py --all > /tmp/<id>-all.txt 2>&1` 실행 후 rc/pass count/failure-marker count만 보고 + 포커스 체커 green + **변이 RED 직접 확인**.
...
- **Brick(계약)**: 이 실패가 계약의 틈(허용돼 있었음) 때문인가, 명시했는데 무시됐기
  ...
  읽은 범위가 write_scope보다 좁지 않았나?(returns-persistence 1차, brick_protocol/support/recording/
  누락으로 5라운드 허비)
...
0702 실측 정정: `brick_protocol/support/connection/adapter_local_cli.py:518`이 `output_excerpt`를
...
| **워크트리 reap 파괴 (0702 사건, 0710 수리)** | 오래된 engine marker + worktree 경로 소멸 | 0710 이후 reaper는 repo 소유권·PID/lease·dirty/WIP를 검증한다. live/cross-repo/불확실 상태는 보존하고, dirty는 마지막 WIP anchor가 검증된 경우에만 제거한다. 실패 시 `recovery_handle`과 worktree를 보존한다. |
| **미완 처분 작업물 유실 (0702 사건, 0710 수리)** | non-complete + 빈 `commit_sha` | 0710 이후 tracked WIP는 검증된 `refs/brick/wip/<building_id>`와 `{ref,sha,base,resume_command}`로 반환된다. anchor 불가(ignored-only·dirty submodule·민감경로 포함)면 dispose를 거부하고 worktree를 남긴다. 공식 resume는 이 WIP 바이트를 재수화한다. |

## 3.3 처분 실행 규칙

- forward/stop/raise/reroute 처분은 JSON으로 선언하고 `brick resume --decl <JSON>`을 실행한다. 처분 행은 격리 작업장 생성 뒤 기록되며, resume/anchor/commit/dispose가 한 bracket에서 닫힌다.
- 공식 CLI에서 `adapter_cwd`를 직접 고르지 않는다. 옛 residue·`/tmp` 선택기는 퇴역했으며 side effect 없는 compatibility seam만 남는다.
- resume 전 `set -a; source ~/.brick/report.env; set +a` — 벨+대시보드 자동.
- **예산 주입**: resume declaration의 `action="raise"`, `budget_increment=N`을 사용한다. `budget_exhausted`만 raise 대상이고, concern 접수·완주는 `forward`, 구현 갭의 지정 수리는 `reroute`+`reroute_target_ref`+`re_instruction`이다.
