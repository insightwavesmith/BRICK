...
Movement authority를 소유하고, brick_protocol/support/model/checker/Slack은 source truth나
...
`compose_building()`, Rule 9)로 들어간다. `build()`, `fan()`, `assemble()`,
...
`graph_packet` JSON(`brick build --graph <packet>` / `support.operator.cli build --graph <packet>` / `brick_protocol.support.operator.cli build --graph <packet>`)은
...
## 과소-폭 금지 규칙 (0703 실측 — 흉터의 과잉일반화 경계)

- **fan을 렌즈 전용으로 오독하지 마라.** fan은 범용 병렬 스테이지다(결정표 2행) —
  렌즈 fan은 그 한 용도일 뿐. "다이어트 그래프" 교훈의 원문은 "불필요한 **직렬 판단**
  노드(blind design)를 빼라"지 "그래프를 좁혀라"가 아니다.
- **대형 읽기 조사(수천 줄 파일·다수 소스)는 존 파티션 fan이 기본형**: 발주자가 존
  경계(라인 범위·파일 목록)를 계약에 박고 `fan([존 리더 ×K(read-only KIND), 표면/pin
  스캐너 …]) → fan(code-attack-qa 재계수, review) → closure(합성)`. 단일 work 레인의
  직렬 정독은 분할 불가능한 일의 특수형이다(0703 실측: 11k줄 조사를 단일 레인에
  태움 — 워크플로였으면 존 리더 6+스캐너 2+표면 1 병렬로 그렸을 일).
- 파티션 병렬의 안전 조건: 읽기 전용이거나 write_scope가 가지별로 분리될 것.
  같은 파일에 쓰는 가지들은 병렬 금지(직렬 슬라이스).
...
`brick_protocol/brick/templates/presets/*.md`(28개 프리셋)은 **미리-사이징된 참조 모양**이고, 그 안의
...
`brick_protocol/support/operator/run.py` walker → active vessel evidence root → reporter/Slack/frontier로 보낸다.
