...
which brain (``preferred_model_ref``). It owns:
  ...
  * ``CASTING_FIELDS`` (moved from ``brick_protocol/support/operator/primitives.py`` at E2/S6),
...
property whose DATA still physically lives in ``brick_protocol/support/connection/agent_adapter``
...
# ---------------------------------------------------------------------------
# CASTING AUTHORING (E2/S1, moved from brick_protocol/support/operator/assembly.py). The builder
...
# Brick verb (``brick_protocol/brick/spec.py:brick``) imports ``_build_casting_bag`` /
...
# Back-compat import contract: brick_protocol/brick/spec.py partitions kwargs by
...
def _expand_llm_alias(
    ...
    The declaration table lives in brick_protocol/support/operator/provider_registry.py; this
...
# Per the E2/S1 plan they STAY in ``brick_protocol/support/operator/assembly.py`` (the graph
...
# brick_protocol/support/connection/agent_resources.py:_AGENT_OBJECT_KEYS / _REF_FIELDS /
...
# connection file (and mirror-copied into brick_protocol/support/operator/primitives.py +
# brick_protocol/support/operator/native_dispatch.py) — definition belonging on the axis. This
...
# (check_axis_field_set_single_source) REDs any literal copy under brick_protocol/support/.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AGENT AUTHORING (E2/S2, moved from brick_protocol/support/operator/assembly.py). The Agent
...
# lower time to ``agent-object:dev`` -> brick_protocol/agent/objects/dev.yaml; the only authoring
