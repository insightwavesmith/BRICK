...
These refs are brick_protocol/support/declaration evidence only. They are not source truth,
...
are brick_protocol/support/operator notes only; they may help operate the work, but they must
...
```text
brick_protocol/agent/ = COO source resources
brick_protocol/support/connection/agent_resources.py = renderer/toolkit
brick_protocol/support/connection/coo_sync.py = Codex / Claude projection writer
brick_protocol/support/connection/mcp_projection.py = read-only MCP call door
~/.codex/skills/brick-protocol-coo/SKILL.md = generated Codex projection
```

If a projection or MCP output differs from brick_protocol/agent/, brick_protocol/agent/ remains the source
