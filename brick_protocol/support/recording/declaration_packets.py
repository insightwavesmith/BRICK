...
# accumulated write (brick_protocol/support/operator/walker_kernel.py / walker_resume.py inject
...
_DECLARED_PLAN_REVISION_READER_DISPOSITION_ROWS: tuple[Mapping[str, str], ...] = (
    ...
    {
        "path": "brick_protocol/support/checkers/check_evidence_spine_projection.py",
    ...
    {
        "path": "brick_protocol/support/operator/walker_carry.py",
        "reader": "_write_declaration_work_evidence caller",
        "disposition": "base_preserved_by_callee",
    ...
    },
    {
        ...
        "reason": "this Brick write_scope forbids brick_protocol/support/operator/run.py mutation",
    },
    {
        "path": "brick_protocol/support/operator/reporter.py",
...
def _is_physical_brick_spec_ref(ref: str) -> bool:
    ...
    if (
        len(path.parts) != 6
        or path.parts[0] != "brick_protocol"
        or path.parts[1] != "brick"
        or path.parts[2] != "templates"
        or path.parts[3] != "bricks" or not path.parts[4]
        or path.parts[5] != "brick.md"
    ):
        return False
...
def _is_physical_brick_template_ref(ref: str) -> bool:
    ...
    if path.is_absolute() or ".." in path.parts:
        return False
    if len(path.parts) < 3 or path.parts[0] != "brick_protocol" or path.parts[1] != "brick":
        return False
    return (_REPO_ROOT / path).is_file()
...
def _plan_snapshot(plan_ref: str, plan: Mapping[str, Any]) -> dict[str, Any]:
    ...
    -- support RECORDS it; the Brick-owned plan under brick_protocol/brick/building_plans/ stays
