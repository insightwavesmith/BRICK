...
`goal-approve` are brick_protocol/support/operator helpers or advanced/internal paths. They
...
| --- | --- | --- |
| What reads first? | Start at repo-root `README.md`, then this `brick_protocol/support/docs/references/quickstart.md`, then `brick_protocol/support/docs/references/setup.md` when you need prerequisite and checker details. After `brick init`, read the generated `FIRST_USE.md` under the chosen output root. | `README.md` links this quickstart and setup guide. `brick_protocol/support/operator/first_use.py` renders `FIRST_USE.md` after the local example. Whether a customer understands the sequence without help is not proven. |
| Active checkout or frozen/history repo? | Use this product checkout or its release export for active work. The frozen HISTORY repository is for archived evidence only; do not start new customer work there. In this checkout, `brick status` reports the resolved `repo_root`, current `cwd`, entrypoint file, and the default evidence root used by `brick build` when `--output-root` is omitted. Release exports omit `project/`; the first onboard/run creates local project evidence as needed. | `README.md` documents release export and HISTORY separation. `brick_protocol/support/operator/cli.py` implements `brick status`. Byte-for-byte release parity and fresh-machine behavior remain not proven until measured. |
...
| How do I read frontier output? | `brick build` exit 0 means the CLI returned support evidence. Customer-visible Building closure is `frontier_kind=complete`. Any other `frontier_kind` is `not_ready`; inspect the printed `evidence_root` before rerunning or escalating. | `brick_protocol/support/operator/cli.py` renders `customer_visible_frontier_state=frontier_complete` only for `complete`; non-complete frontiers render `not_ready` and preserve `evidence_root`. This is evidence handling, not a phase PASS or quality judgment. |
...
`brick_protocol/support/operator/walker_resume.py::hold_disposition_action_menu`; the approve
...
| --- | --- | --- | --- |
| `agent_incomplete` or `link_paused` | `adapter_error_frontier` (`brick_protocol/support/operator/walker_resume.py::_adapter_error_hold_without_return`) | `stop` | `forward`, `raise`, `reroute` |
| `link_paused` | `target_node_budget_exhausted` or `budget_exhausted=True` (`brick_protocol/support/operator/walker_resume.py::hold_disposition_action_menu`) | `raise` with positive `budget_increment`, `stop`, or explicit `reroute` | `forward` |
| `human_review_waiting` | `fake_landing_write_scope_diff_absent` or `write_scope_forbidden_diff_present` (`brick_protocol/support/operator/frontier_observation.py::_latest_hold_reason`) | `forward`, `stop`, or explicit `reroute` | `raise` |
| `link_paused` | `human_or_coo_gate_pause` (`brick_protocol/support/operator/walker_resume.py::hold_disposition_action_menu`) | `forward`, `stop`, or explicit `reroute` | `raise` |
...
command: cd ~/BRICK && PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=brick_protocol/support/import_identity uv run python3 brick_protocol/support/operator/dashboard_export.py --bake-public
expected: "baked brick_protocol/support/dashboard/public/dashboard-data.json | buildings N | source_truth False".
failure signal: source_truth가 False가 아니거나, dashboard-data.json 생성 실패, 또는 project evidence shape 거절.

step: checker gate
command: cd ~/BRICK && PYTHONPATH=brick_protocol/support/import_identity uv run python3 brick_protocol/support/checkers/check_profile.py --all
...
| 증상 | 처방 |
| --- | --- |
| `ModuleNotFoundError: No module named 'brick_protocol'` (또는 `'yaml'`) | 저장소 루트에서 `uv run python3 ...` 형식으로 실행 (uv 없이는 `PYTHONPATH=brick_protocol/support/import_identity python3 ...` + 전역 PyYAML 필요) |
...
underlying brick_protocol/support/operator helper materializes the selected chain preset into
...
`brick_protocol/brick/building_plans/onboarding-example-0.yaml`; point the runner at it rather
...
```bash
uv run python3 -c 'from brick_protocol.support.operator.run import run_building_plan; result = run_building_plan("brick_protocol/brick/building_plans/onboarding-example-0.yaml"); print(result.building_id); print(result.lifecycle_write.root); print("\n".join(str(path) for path in result.written_files))'
```

No `uv`? The bare-Python alternative works only if PyYAML is installed for your global `python3`:

```bash
PYTHONPATH=brick_protocol/support/import_identity python3 -c 'from brick_protocol.support.operator.run import run_building_plan; result = run_building_plan("brick_protocol/brick/building_plans/onboarding-example-0.yaml"); print(result.building_id); print(result.lifecycle_write.root); print("\n".join(str(path) for path in result.written_files))'
```
...
can still use the brick_protocol/support/operator helper with a repo-relative
...
(No `uv`? Replace `uv run python3` with `PYTHONPATH=brick_protocol/support/import_identity python3` — that needs PyYAML installed for your global `python3`.)

Intent dict fields (grounded in
`brick_protocol/support/operator/composition_intent.py::materialize_building_intent` and the
...
- `task_source_ref` (automation flow) — safe repo-relative path to an
  ...
  `brick_protocol/brick/templates/tasks/`). Must exist; its sha256 is recorded. Mutually
