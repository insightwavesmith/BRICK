...
(`brick_protocol/support/operator/assembly.py`) + `run_building_plan()`입니다. 손으로 작성한
`graph_packet` JSON을 넘기는 `brick build --graph <packet.json>`은 Global
Operating Rule 10에 따라 retired입니다. `sibling_independence`, 노드별
`write_scope` narrowing, mid-graph human/COO gates는 이제 DSL로 표현합니다.
...
DSL graph
  -> assemble() / build() / fan()                          [공식 authoring DSL]
  -> support가 DSL graph를 Building Plan으로 materialize하고 선언된 그래프를 걸음
  -> evidence_root, frontier_kind, proof_limits를 출력
```
...
경우 caller/COO가 DSL graph로 길을 얼린 뒤 공식 DSL 경로로 실행합니다:
...
DSL graph를 선언합니다. support는 그 선언을 materialize하고 걷지만, route
target, Movement, 성공, 품질을 고르지 않습니다.
...
- Official graph authoring/launch DSL: `assemble()`, `build()`, `fan()`.
...
`proposed-building-graph.json` 위에서만 동작). 이 helper는 운영자/내부 자동화
경로입니다. 고객-facing 실행 route로 문서화하거나 처음 실행할 때는 선언된
graph packet CLI handoff를 사용하지 않습니다:
