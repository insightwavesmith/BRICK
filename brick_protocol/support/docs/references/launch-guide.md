...
```bash
brick build --task "..." --building-case quick_fix --intensity easy --direct-preset --fast-confirm
# fresh clone / entrypoint not guaranteed:
uv run python3 -m brick_protocol.support.operator.cli build --task "..." --building-case standard_delivery --intensity normal
```

task가 있으면 caller/COO는 반드시 다음 하나를 명시합니다.

- 이미 검토한 preset: `--preset building-chain-preset:...`
- Building Call triage: `--building-case ... --intensity ...`
- 검토·확인된 Building Call JSON: `--building-call ...`

task만 있고 이 선언이 없으면 CLI는 write-capable adapter 여부와 무관하게 실행 전
fail-closed합니다. preset 후보 랭킹은 advisory일 뿐 자동 선택하지 않습니다.
`quick_check`/`quick_fix`만 `--direct-preset --fast-confirm`과 함께 직접 lowering되고,
나머지 easy 및 normal/complex/critical 요청은 draft-only
`building-call-authoring` preset을 먼저 실행합니다. 확인된 JSON은
`lower_building_call_request_v1_1`을 거쳐 같은 customer sandbox dispatch로 갑니다.

`--graph-decl`은 기존 canonical graph 선언의 **호환 transport**로 유지합니다.
`assemble()` / `build()` / `fan()`과 `run_building_plan()`은 Builder 내부 재료이며
별도 고객 실행 입구가 아닙니다. 손으로 작성한 raw `graph_packet`의
`--graph`/`--graph-packet` 입력은 retired입니다.

`run_building_intake`, `launch_assembled_building`, and `goal-approve` are
brick_protocol/support/operator helpers or advanced/internal paths. They are not separate
...
```
task(말로 전한 일)
  -> brick build --task                                   [고객 표면]
  -> explicit preset OR Building Call triage
  -> quick direct lowering OR authoring Building
  -> confirmed Building Call canonical lowering
  -> support runner가 선언된 Brick / Agent / Link row를 격리 워크트리에서 걸음
  -> evidence_root, frontier_kind, proof_limits를 출력

compat graph declaration
  -> brick build --graph-decl ...                          [호환 transport]
  -> 같은 declared-plan / sandbox lifecycle
```
...
- **STRUCTURE = declared input**: preset_task는 선언된 프리셋이 구조를 주고,
  DSL graph는 caller/COO가 이미 선언한 그래프가 구조를 줍니다.
...
큰 작업도 별도 CLI 입구가 아닙니다. 고객/운영자는 일을 말하고 명시 triage로
draft-only authoring Building을 먼저 실행합니다:
...
make X
  ...
  -> COO confirm
  -> canonical lowering
  -> declared-plan dispatch
```

프리셋이 이 계약을 그대로 보존하고 caller/COO가 이미 검토했을 때만
`brick build --task ... --preset ...`를 씁니다. 설계 분기, 병렬 구현, lane QA,
최종 QA가 필요한 일은 authoring Building이 초안을 만들고 COO가 확인한 뒤
canonical lowering합니다. support는 선언을 materialize하고 걷지만 route target,
Movement, 성공, 품질을 고르지 않습니다.

Public route vocabulary:

- Customer/operator: `brick build --task` and
  `uv run python3 -m brick_protocol.support.operator.cli build ...` when the
  entrypoint is not installed yet.
- Compatibility transport: `brick build --graph-decl <decl>`; not taught as a new entrance.
- Builder/internal graph materials: `assemble()`, `build()`, `fan()`.
- Advanced/internal helpers: `run_building_intake`,
...
The release gate is a brick_protocol/support/onboarding check, not a Building route:
...
brick build \
  ...
  --preset building-chain-preset:quick-check \
  --fast-confirm \
...
observed-write adapter를 고른 다음 실행합니다:
...
`--real-provider`는 caller가 명시한 exact `--adapter`의 readiness만 관찰합니다.
로그인 상태를 보고 다른 performer를 고르지 않습니다. 선언이 없거나 선언한
provider가 준비되지 않았으면 Building 실행 전 typed stop으로 끝납니다. non-quick
provider 작업은 조용한 120초 기본값을 쓰지 않으며 양수 `--timeout` 선언도 필요합니다. 단
verdict-bearing design/review/closure 노드는 local adapter만으로 complete가 아니라
`agent_incomplete`/`not_ready`를 반환할 수 있습니다.

### 2) 선언된 그래프: DSL

선언된 그래프는 `assemble()` / `build()` / `fan()` DSL로 조립합니다. raw
`graph_packet` JSON을 `brick build --graph`로 넘기는 CLI 입력은 retired입니다.
support는 DSL이 materialize한 plan을 걷습니다. support가 route target,
Movement, 성공, 품질을 발명하지 않습니다.

### Advanced/internal operator helper: assembly

`brick_protocol/support/operator/assembly.py` 의 핸들 기반 API로 그래프를 선언합니다.
브릭 종류·일·누구만 말하면 됩니다 (ref/id는 빌더가 채움). 이 API는
brick_protocol/support/operator helper입니다. raw graph JSON CLI handoff가 아니라 이 DSL이
공식 graph authoring 경로입니다:
...
### Advanced/internal operator helper: approval
...
미리보기와 canonical graph declaration을 확인한 뒤, 호환 transport가 필요한 기존
운영은 공식 CLI 안에서만 실행합니다:

```bash
brick build --graph-decl /path/to/canonical-graph-decl.yaml --action forward
```

`python -m brick_protocol.support.operator.onboard`는 봉인된 비공식 모듈 CLI이며
직접 호출하지 않습니다. `--action stop`이면 proposal만 기록하고, `forward`는
선언된 플랜을 customer sandbox lifecycle로 보냅니다.
`render_proposal_for_human` 과 `run_goal_approve_entry` 는
`brick_protocol/support/operator/onboard.py` 에 있습니다.

### Internal operator helper: intake seam

`run_building_intake`는 `brick build --task/--preset` 아래의 brick_protocol/support/operator
helper입니다. 고객 문서에서는 이 helper를 별도 실행 route로 세우지 않습니다.
운영자나 테스트가 직접 호출할 때는 프리셋을 골라 intent를 넘깁니다. 고객
first-run 또는 일반 실행 안내에서는 이 snippet 대신 `brick build --task`를
사용하세요:
...
공식 고객 실행 표면은 하나입니다: `brick build`. 그 아래 CLI 입력 모드는
`preset_task`(`--task`/`--preset`)입니다. 그래프형 작업은 DSL authoring 경로로
선언합니다.
...
- ❌ **`/tmp` 에 스크립트 파일을 만들어 직접 PYTHONPATH 를 박지 마세요.**
  ...
  (uv 가 없으면 `PYTHONPATH=brick_protocol/support/import_identity python3 ...` 를 루트에서
...
- **fast-fix** (`brick_protocol/brick/templates/presets/fast-fix.md`): work → code-attack-qa →
  closure. COO 게이트가 없어 사람 개입 없이 자율로 굴러갑니다. work 단계가
  막히면 reroute 예산(5) 안에서 스스로 되돌아갑니다.
- **engine-feature-hard** (`brick_protocol/brick/templates/presets/brick-protocol-engine-feature-hard.md`):
...
PYTHONPATH=brick_protocol/support/import_identity python3 brick_protocol/support/checkers/check_profile.py --all
...
- `onboard goal "<text>"` CLI — 제거됨. (혼동 주의: `onboard goal-approve` 는
  남아 있는 brick_protocol/support/operator helper입니다. `goal`(제거)과
  `goal-approve`(helper)는 다릅니다.)
- `brick_protocol/support/operator/auto_compose.py` / `auto_compose` — 제거됨.
