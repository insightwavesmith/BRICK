...
current tree. The authoritative census is `brick_protocol/support/checkers/module_registry.yaml`
...
`brick_protocol/support/operator/` families plus the surrounding layers.

## Module families (brick_protocol/support/operator, one folder)
...
```text
brick_protocol/support/recording/    deterministic evidence writers (capture, raw, claims, spine)
brick_protocol/support/connection/   Agent-brain / adapter / MCP / sync connection surfaces
brick_protocol/support/checkers/     profile runner + kernel checks + declarative profiles
brick_protocol/support/onboarding/   install script + provider-native open/close hooks
```
...
task.md / inline statement (+ optional project_ref)
        |
        v
run_building_intake                brick_protocol/support/operator/driver.py
  read task, confirm declared intent
        |
        v
materialize_building_intent        brick_protocol/support/operator/composition_intent.py
  ...
  writes work/declared-building-plan.json under the building root
        |
        v
run_building_plan                  brick_protocol/support/operator/run.py
  walker walks the declared steps (dynamic_walker + walker_* kernel)
  gates measure sufficiency (gate_sequence)
  evidence spine recorded (brick_protocol/support/recording/*)
        |
        v
...
`launch_assembled_building` remain brick_protocol/support/operator helper or advanced/internal
...
```text
module census        brick_protocol/support/checkers/module_registry.yaml
...
building plan        GENERATED artifact: work/declared-building-plan.json inside
                     ...
                     entry (brick_protocol/brick/building_plans/ is the dogfood-era hand-declared
