...
- **PyYAML** — a declared runtime dependency (`pyproject.toml`), installed by `uv sync` / the installer. It is required on EVERY intake/preset run: the operator surface loads the split brick catalog and preset registry via YAML (`brick_protocol/support/operator/plan_rendering.py`), not only when your own plan file is `.yaml`/`.yml`. (A hand-written JSON plan passed straight to `run_building_plan` is the one path that does not touch it.)

The Python import identity lives under `brick_protocol/support/import_identity/brick_protocol/`. The package is not installed into site-packages, so every command below puts that directory on `PYTHONPATH`. The namespace is `brick_protocol`.
...
A green gate is **exit code 0**. On success the runner prints one `profile passed:` line per active profile, then the closing proof-limit line. Measure the current profile count from the checkout instead of copying a fixed number:

```bash
find brick_protocol/support/checkers/profiles -maxdepth 1 -name '*.yaml' | wc -l
```
...
`check_profile.py --all`, then `brick_protocol/support/onboarding/release_export.sh` into a
...
```bash
PYTHONPATH=brick_protocol/support/import_identity python3 brick_protocol/support/checkers/check_profile.py --profile core
```

The profile name resolves against `brick_protocol/support/checkers/profiles/`. Current profile names include `core`, `building_automation`, `agent_axis_behavioral`, `link_routing_behavioral`, `tier_a_three_axis_conformance`, and others — list them with `ls brick_protocol/support/checkers/profiles/`.
...
```bash
PYTHONPATH=brick_protocol/support/import_identity python3 brick_protocol/support/checkers/check_profile.py --self-test
```

Inspect a repo other than the current directory:

```bash
PYTHONPATH=brick_protocol/support/import_identity python3 brick_protocol/support/checkers/check_profile.py --all --repo /path/to/repo
```
...
The lower-level brick_protocol/support/operator helpers (`run_building_intake`, `assemble`,
...
`brick_protocol/brick/building_plans/onboarding-example-0.yaml` (it is GRAPH-shaped, which the
...
`.yaml` / `.yml` file. Its real signature (from `brick_protocol/support/operator/run.py`) is:
...
walker admission guard (`brick_protocol/support/operator/walker_kernel.py`) with a
`ValueError`. The bundled `brick_protocol/brick/building_plans/onboarding-example-0.yaml` is
