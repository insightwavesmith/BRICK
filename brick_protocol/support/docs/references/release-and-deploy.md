...
```bash
sh brick_protocol/support/onboarding/release_export.sh --output /tmp/BRICK-v0.1.0
```
...
npm ci
npm run build
# Vercel: root=brick_protocol/support/dashboard, build=npm run build, output=dist
```
...
PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=brick_protocol/support/import_identity \
  python3 brick_protocol/support/operator/dashboard_export.py --bake-public
docker build -t brick-dashboard:local brick_protocol/support/dashboard
docker run --rm \
...
[`brick_protocol/support/dashboard/DEPLOY.md`](../../dashboard/DEPLOY.md)를 보세요.
