...
from brick_protocol.support.checkers.lib.yaml_subset import ProfileError, parse_yaml_subset
...
OLD_REGISTRY_PATH = "brick_protocol/brick/templates/shapes/registry.yaml"
SPLIT_CATALOG_PATHS = {
...
BRICKS_SPEC_DIR = "brick_protocol/brick/templates/bricks"
...
PRESETS_SPEC_DIR = "brick_protocol/brick/templates/presets"
SHAPES_SUBTREE_DIR = "brick_protocol/brick/templates/shapes"
...
# human<->COO design-contract table lives at the brick_protocol/brick/templates/ root. The old
...
ADMITTED_TEMPLATE_ROOT_FILES = {
    ...
    "brick_protocol/brick/templates/building-design-contract.yaml",
    "brick_protocol/brick/templates/reroute-defaults.yaml",
}
...
# its physical brick_protocol/brick/templates path and a stale/retired ref REDs loudly (the
...
def _gate_registry_rows(repo: Path) -> tuple[tuple[str, str | None], ...]:
    ...
    ``GATE_REGISTRY`` in ``brick_protocol/link/spec.py`` (one ``GateRegistryRow`` per gate);
    ``brick_protocol/link/gate.py``'s ``DECLARED_GATE_REFS`` is now DERIVED from it
    ...
    reads each row's ``ref=`` string in registry order from ``brick_protocol/link/spec.py``.
    ...
    """
    path = repo / "brick_protocol/link/spec.py"
    tree = ast.parse(path.read_text(encoding="utf-8"), filename=str(path))
...
def _gate_refs_from_gate_yaml(repo: Path) -> tuple[str, ...]:
    payload = _as_mapping(_load_structured(repo / "brick_protocol/link/gate.yaml"))
    refs: list[str] = []
...
def _support_gate_refs(repo: Path) -> tuple[str, ...]:
    path = repo / "brick_protocol/support/operator/building_operation_common.py"
...
def _is_brick_kind_template_dir(path: str) -> bool:
    ...
    return (
        len(parts) == 5
        and parts[:4] == ("brick_protocol", "brick", "templates", "bricks")
        and _slug_part(parts[4])
    )
    ...
    for rel_dir in ADMITTED_PHYSICAL_TEMPLATE_DIRS:
        ...
        for path in root.iterdir():
            ...
            if suffix in ADMITTED_TEMPLATE_SUFFIXES or (
                rel_dir == "brick_protocol/brick/templates/tasks" and suffix in TASK_TEMPLATE_SUFFIXES
            ):
...
def _archived_template_files(repo: Path) -> set[str]:
    ...
    ref that exists at no physical brick_protocol/brick/templates path REDs.
    """
    del repo
    return set()
...
def _chain_preset_rows(
    ...
    for rel, document, compat_old in documents:
        ...
        for row in rows:
            ...
            # protocol.yaml / brick_protocol/brick/templates/buildings|portfolios) were dead and
            ...
            if concept and concept not in gate_concepts:
                violations.append(
                    CatalogViolation(
                        "invalid_chain_gate_concept_profile",
                        location,
                        f"gate_concept_profile item {concept} is absent from brick_protocol/link/gate.yaml concepts",
                    )
                )
            elif not concept:
                violations.append(
...
def _validate_declaration_packets(
    ...
    for rel, packet in packets:
        ...
        for ref in _as_list(packet.get("legacy_expanded_brick_template_refs")):
            ...
            if ref_text.startswith("building-step-template:"):
                continue
            if ref_text.startswith("brick_protocol/brick/templates/"):
                problem = _validate_declared_template_ref(
                    ref_text, physical_files, archived_template_files
                )
                if not problem:
        ...
        if _looks_like_return_template(document):
            violations.append(
                CatalogViolation(
                    "shape_subtree_return_template_not_admitted",
                    rel,
                    "return-template shaped files must live under admitted physical template directories, not brick_protocol/brick/templates/shapes",
                )
            )
        for value_path, field in _return_field_declarations(document):
            normalized = _normalized_field_name(field)
...
def _c2_template_path(path: str) -> str:
    if path.startswith("brick/templates/"):
        return "brick_protocol/" + path
    return path


def _template_path_shape_problem(path: str) -> str | None:
    """Shared path-shape rules for ANY brick_protocol/brick/templates ref (live or historical)."""
    path = _c2_template_path(path)
    if path.startswith("/") or ".." in Path(path).parts:
        return "physical template path must stay inside the repo"
    if not path.startswith("brick_protocol/brick/templates/"):
        return "physical template path must stay under brick_protocol/brick/templates"
    ...
    if suffix not in ADMITTED_TEMPLATE_SUFFIXES and not (
        parent == "brick_protocol/brick/templates/tasks" and suffix in TASK_TEMPLATE_SUFFIXES
    ):
...
def _validate_physical_path(path: str, physical_files: set[str]) -> str | None:
    path = _c2_template_path(path)
    """LIVE binding resolution: the file must exist in the live brick_protocol/brick/templates tree.
...
def _validate_declared_template_ref(
    ...
    brick_protocol/brick/templates path. The 0610 archive-mirror fallback is pruned with the
    ...
    if path in physical_files or path in archived_template_files:
        return None
    return (
        "template file does not exist at its brick_protocol/brick/templates path (no archive "
...
def _validate_brick_spec_path(path: str, brick_spec_files: set[str]) -> str | None:
    ...
    if (
        len(parts) != 6
        or parts[0] != "brick_protocol"
        or parts[1] != "brick"
        or parts[2] != "templates"
        or parts[3] != "bricks"
        or not _slug_part(parts[4])
        or parts[5] != BRICK_SPEC_FILENAME
    ):
...
def _validate_state(
    ...
    *,
    ...
    for source_name, observed in (
        ("brick_protocol/link/gate.yaml declared_gate_refs", tuple(state.get("declared_gate_refs_yaml", ()))),
        ...
        if set(observed) != canonical_gate_ref_set:
            violations.append(
                CatalogViolation(
                    "link_gate_token_drift",
                    source_name,
                    f"observed {sorted(observed)} but brick_protocol/link/gate.py declares {sorted(canonical_gate_refs)}",
                )
            )
        ...
        if catalog_gate_refs and catalog_gate_refs != compact_gate_ref_set:
            violations.append(
                CatalogViolation(
                    ...
                    "brick_protocol/link/gate.py DECLARED_GATE_REFS",
                )
            )
        for value_path, value in _walk_values(document):
        ...
        if agent_ref and agent_ref not in agent_refs:
            violations.append(
                CatalogViolation(
                    "agent_object_ref_payload_drift",
                    location,
                    f"agent_object_ref {agent_ref} is absent from brick_protocol/agent/objects payload object_ref values",
                )
            )
    ...
    closure_doc = _as_mapping(
        physical_template_docs.get(
            "brick_protocol/brick/templates/bricks/closure/return.yaml"
        )
    )
...
def _live_state(repo: Path) -> Mapping[str, Any]:
    ...
    return {
        ...
        "movement_literals": tuple(_py_constant(repo / "brick_protocol/link/movement.py", "MOVEMENT_LITERALS")),
...
def _synthetic_base(temp_root: Path) -> Mapping[str, Any]:
    physical_files = {
        "brick_protocol/brick/templates/bricks/code-attack-qa/return.yaml",
        "brick_protocol/brick/templates/bricks/work/return.yaml",
        "brick_protocol/brick/templates/bricks/closure/return.yaml",
    }
    brick_spec_files = {
        "brick_protocol/brick/templates/bricks/code-attack-qa/brick.md",
        "brick_protocol/brick/templates/bricks/work/brick.md",
        "brick_protocol/brick/templates/bricks/closure/brick.md",
    }
    ...
    catalog = {
        "compact_link_authoring_view": {
            ...
            "canonical_link_source": "brick_protocol/link/gate.py DECLARED_GATE_REFS",
    ...
    return {
        "seed_violations": [],
        "catalogs": [
            ("brick_protocol/brick/templates/shapes/catalog.yaml", catalog, False),
            ("brick_protocol/brick/templates/shapes/shapes.yaml", shapes_doc, False),
        ...
        "brick_template_ref_map": {
            "brick-template:code-attack-qa-return": "brick_protocol/brick/templates/bricks/code-attack-qa/return.yaml",
            "brick-template:work-return": "brick_protocol/brick/templates/bricks/work/return.yaml",
            "brick-template:closure-return": "brick_protocol/brick/templates/bricks/closure/return.yaml",
        },
        "active_inputs": ("brick_protocol/brick/templates/shapes/catalog.yaml",),
        "compat_inputs": (),
...
def _run_fire_fixtures() -> tuple[int, list[str]]:
    ...
    mutations: dict[str, tuple[str, str, Any]] = {
        ...
        "invalid_physical_template_path": (
            "invalid_physical_template_path",
            "set_binding",
            "brick_protocol/brick/templates/shapes/not-a-physical-return.yaml",
        ),
        "step_ref_masquerades_as_brick_template_ref": (
            "step_ref_masquerades_as_brick_template_ref",
            "set_binding",
        ...
        # that exists at NEITHER brick_protocol/brick/templates nor archive/brick-templates
    ...
    with tempfile.TemporaryDirectory(prefix="brick-template-catalog-fixtures-") as tmp:
        ...
        for fixture_name, (expected_code, mutation, value) in mutations.items():
            ...
            if mutation == "remove_binding":
                ...
                state["physical_files"] = set(state["physical_files"]) | {"brick_protocol/brick/templates/review/orphan.yaml"}
                ...
                packet["expanded_brick_template_refs"] = [
                    "brick_protocol/brick/templates/review/never-existed-return.yaml"
                ]
                ...
                docs["brick_protocol/brick/templates/bricks/work/return.yaml"]["required_return_shape"].append(
                    "movement_choice"
                )
            elif mutation == "axis_owned_return_top_level_key":
                docs = state["physical_template_docs"]  # type: ignore[index]
                docs["brick_protocol/brick/templates/bricks/work/return.yaml"]["movement_choice"] = "reroute"
            elif mutation == "axis_owned_next_link_input_section":
                docs = state["physical_template_docs"]  # type: ignore[index]
                docs["brick_protocol/brick/templates/bricks/work/return.yaml"]["next_link_input_sections"] = [
                    "performer"
                ]
                ...
                state["shape_subtree_extra_documents"] = (
                    (
                        "brick_protocol/brick/templates/shapes/bad-return-template.yaml",
                        {
                ...
                state["shape_subtree_extra_documents"] = (
                    (
                        "brick_protocol/brick/templates/shapes/innocent-return-template.yaml",
                        {
...
def run_check(repo: Path, mode: str) -> tuple[int, int, int]:
    ...
    print(
        "- anchors: brick_protocol/link/gate.py DECLARED_GATE_REFS; "
        "brick_protocol/agent/objects/*.yaml payload object_ref; "
        "brick_protocol/link/movement.py MOVEMENT_LITERALS; "
