...
PHASE4_PROJECTIONS = {
    "brick_protocol/brick/work.yaml": "Brick work YAML",
    "brick_protocol/agent/return_fact.yaml": "Agent return_fact YAML",
    "brick_protocol/link/movement.yaml": "Link movement YAML",
    "brick_protocol/agent/receipt.yaml": "Agent receipt YAML",
    "brick_protocol/agent/performance.yaml": "Agent performance YAML",
    "brick_protocol/brick/building.yaml": "Brick building YAML",
    "brick_protocol/brick/comparison.yaml": "Brick comparison YAML",
    "brick_protocol/link/gate.yaml": "Link gate YAML",
    "brick_protocol/link/transition.yaml": "Link transition YAML",
    "brick_protocol/link/transfer.yaml": "Link transfer YAML",
    "brick_protocol/link/carry.yaml": "Link carry YAML",
}
...
def check_link_transition(path: Path, lines: list[str]) -> list[str]:
    ...
    # "Movement: return|hold|stop|pass" on brick_protocol/link/transition.yaml). Run the SAME
...
def check_file(path: Path) -> tuple[list[str], bool]:
    ...
    if projection == "brick_protocol/brick/work.yaml":
        return check_brick_work(path, lines), True
    if projection == "brick_protocol/agent/return_fact.yaml":
        return check_agent_return_fact(path, lines), True
    if projection == "brick_protocol/link/movement.yaml":
        return check_link_movement(path, lines), True
    if projection == "brick_protocol/agent/receipt.yaml":
        return check_agent_receipt(path, lines), True
    if projection == "brick_protocol/agent/performance.yaml":
        return check_agent_performance(path, lines), True
    if projection == "brick_protocol/brick/building.yaml":
        return check_brick_building(path, lines), True
    if projection == "brick_protocol/brick/comparison.yaml":
        return check_brick_comparison(path, lines), True
    if projection == "brick_protocol/link/gate.yaml":
        return check_link_gate(path, lines), True
    if projection == "brick_protocol/link/transition.yaml":
        return check_link_transition(path, lines), True
    if projection == "brick_protocol/link/transfer.yaml":
    ...
    if projection == "brick_protocol/link/carry.yaml":
        return check_link_transfer_or_carry(
            path,
            lines,
...
def collect_paths(args: argparse.Namespace) -> list[Path]:
    ...
    if args.repo:
        ...
        if repo.is_file() or repo.name in {"brick", "agent", "link"}:
            ...
            paths.extend(repo / "brick_protocol" / value for value in ("brick", "agent", "link"))
    ...
    if not paths:
        paths.extend(Path("brick_protocol") / value for value in ("brick", "agent", "link"))
