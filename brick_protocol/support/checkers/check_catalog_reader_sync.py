...
Brick template catalog files live under ``brick_protocol/brick/templates/``. Production readers
...
exists. This checker fails when any catalog-file path a brick_protocol/support/ reader names
does not resolve to an existing file.

It catches BOTH a direct string literal and a STATICALLY-CONSTRUCTED path:
``"brick_protocol/brick/templates/" + "x.yaml"``, ``Path("brick_protocol/brick") / "templates" / "x.yaml"``,
...
``brick_protocol/brick/templates/*.yaml`` literal is checked for existence, so an illustrative
...
CATALOG_PREFIX = "brick_protocol/brick/templates/"
SCAN_DIR = Path("brick_protocol/support")
...
FIXTURE_OWNER = "brick_protocol/support/checkers/check_brick_template_catalog_restructure.py"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Support-evidence checker: every catalog-file path a brick_protocol/support/ code "
    ...
    print(
        ...
        f"{checked} catalog-file path reference(s) in brick_protocol/support/ readers resolve."
    )
    return 0
