...
def _check_directory_allowed_path_is_not_recursive(label: str) -> None:
    ...
    if "project/example/work/building-map.json" not in outside:
        raise ProfileError(
            ...
            f"brick_protocol.brick.comparison (directory scope is not recursive; move+record only), "
            f"observed {facts!r}"
        )
...
def _check_token_shaped_filename_short_marker_accepted(label: str) -> None:
    from brick_protocol.support.operator.primitives import _FORBIDDEN_PAYLOAD_KEYS, _validate_no_payload_forbidden
    ...
    try:
        _validate_no_payload_forbidden(
            ...
            {"task_source_ref": "brick_protocol/brick/templates/tasks/sk-demo.md"},
...
def _check_dirty_root_reuse_requires_overwrite_or_new_root(label: str) -> None:
    from brick_protocol.support.operator.run import _preflight_step_output_building_root
