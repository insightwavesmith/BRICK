...
from brick_protocol.support.checkers.lib.no_smith_residue_check import _SMITH_USER_HOME_LITERAL
...
_DASHBOARD_PRODUCTIZATION_SKIP_RELATIVES = {
    "brick_protocol/support/dashboard/package-lock.json",
    "brick_protocol/support/dashboard/public/dashboard-data.json",
}
_DASHBOARD_PUBLIC_DATA_RELATIVE = "brick_protocol/support/dashboard/public/dashboard-data.json"
...
def _dashboard_productization_text_paths(repo: Path) -> list[Path]:
    root = repo / "brick_protocol" / "support" / "dashboard"
    paths: list[Path] = []
...
def _dashboard_productization_assert_non_utf8_skip_probe() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="bp-dashboard-nonutf8-fire-") as tmp:
        probe_repo = Path(tmp)
        dashboard = probe_repo / "brick_protocol" / "support" / "dashboard"
        ...
        if skipped_non_utf8 != ["brick_protocol/support/dashboard/binary.md"]:
            raise ProfileError(
...
def _dashboard_productization_assert_literal_fire_probe() -> int:
    probe_dir = Path("brick_protocol/support/dashboard/DEPLOY.md")
    probe_lines = {
...
def _dashboard_productization_assert_bake_shape_probe(repo: Path) -> int:
    from brick_protocol.support.operator import dashboard_export
...
def _dashboard_productization_assert_signed_ingest_request(request: urllib.request.Request) -> None:
    from brick_protocol.support.operator import report_sinks
...
def _dashboard_productization_assert_openssl_subprocess_scope(repo: Path) -> int:
    report_sinks_path = repo / "brick_protocol/support/operator/report_sinks.py"
...
def _dashboard_productization_assert_state_projection_cases(
    ...
    from brick_protocol.support.operator import dashboard_export, ledger_projection
...
def run_dashboard_productization_projection(repo: Path) -> KernelResult:
    ...
    required_paths = (
        repo / "brick_protocol/support/dashboard/DEPLOY.md",
        repo / "brick_protocol/support/dashboard/server/index.mjs",
    )
    ...
    server_text = (repo / "brick_protocol/support/dashboard/server/index.mjs").read_text(encoding="utf-8")
