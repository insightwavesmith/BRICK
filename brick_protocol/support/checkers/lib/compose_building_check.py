"""Compose-building behavioral profile runners.
...
def run_compose_building_case(repo: Path, profile: Mapping[str, Any]) -> int:
    ...
    if not items:
        return 0
    from brick_protocol.support.operator.building_operation import compose_building, observe_building_frontier
    ...
    from brick_protocol.support.operator.composition_route_policy import _composition_resolve_route_policy_provenance
    ...
    from brick_protocol.support.operator.composition_graph_validate import _composition_policy_action
...
def run_compose_building_rejects(repo: Path, profile: Mapping[str, Any]) -> int:
    ...
    if not items:
        return 0
    from brick_protocol.support.operator.building_operation import CompositionError
