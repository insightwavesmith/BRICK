...
def run_chat_session_park_seam(repo: Path) -> KernelResult:
    ...
    from brick_protocol.support.checkers import check_building_lifecycle_path_shape as lifecycle_shape
    from brick_protocol.support.checkers import check_package_path_admission as path_admission
    from brick_protocol.support.operator import run as run_module
    from brick_protocol.support.operator import (
        dashboard_export,
    ...
    with tempfile.TemporaryDirectory(prefix="bp-chat-session-park-seam-") as tmpdir:
        ...
        shutil.copytree(
            repo / "brick_protocol" / "agent",
            temp_repo / "brick_protocol" / "agent",
        ...
        from brick_protocol.support.checkers.lib.agent_session_id_redaction_check import (
            _SESSION_ID_ULID_RE,
            _SESSION_ID_UUID_RE,
        )
...
def _chat_session_assert_declared_adapter_capability(temp_repo: Path) -> None:
    ...
    if "adapter:chat-session" not in dev.get("adapter_refs", []):
        raise ProfileError(
            "chat_session_park_seam expected brick_protocol/agent/objects/dev.yaml to declare "
    ...
    if "adapter:chat-session" in qa.get("adapter_refs", []):
        raise ProfileError(
            ...
            "brick_protocol/agent/objects/qa.yaml to omit adapter:chat-session"
        )


def _chat_session_agent_object(temp_repo: Path, object_name: str) -> Mapping[str, Any]:
    path = temp_repo / "brick_protocol" / "agent" / "objects" / f"{object_name}.yaml"
    try:
...
def _chat_session_assert_envelope_session_key_rejects(uuid_probe: str) -> None:
    ...
    from brick_protocol.support.recording import adapter_error_frontier
...
def _chat_session_assert_park_evidence(
    ...
    *,
    ...
    from brick_protocol.support.connection.agent_adapter import AgentAdapterRequest
    from brick_protocol.support.checkers import check_building_declaration_integrity as declaration_integrity
