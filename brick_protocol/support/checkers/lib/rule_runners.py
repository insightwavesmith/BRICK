...
def run_agent_resource_boundary(repo: Path, profile: Mapping[str, Any]) -> int:
    ...
    if not items:
        return 0
    from brick_protocol.support.connection.agent_resources import render_agent_packet
...
def run_agent_resource_retired_ref_rejects(repo: Path, profile: Mapping[str, Any]) -> int:
    ...
    for item in items:
        ...
        probe_path = repo / "brick_protocol" / "agent" / "objects" / "retired-ref-probe.yaml"
        try:
...
def run_agent_preferred_adapter_rejects(repo: Path, profile: Mapping[str, Any]) -> int:
    ...
    for item in items:
        ...
        probe_path = repo / "brick_protocol" / "agent" / "objects" / "preferred-adapter-probe.yaml"
        try:
            _validate_agent_authority(
...
def _admitted_agent_object_refs(repo: Path) -> set[str]:
    ...
    if not object_dir.is_dir():
        raise ProfileError("brick_protocol/agent/objects must exist")
    refs = {f"agent-object:{path.stem}" for path in object_dir.glob("*.yaml")}
    if not refs:
        raise ProfileError("brick_protocol/agent/objects must contain admitted Agent Object resources")
    return refs
