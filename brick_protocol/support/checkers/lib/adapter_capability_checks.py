...
def _assert_recorded_write_policy_fact(
    ...
    if not any(expected_reason in fact for fact in facts):
        raise ProfileError(
            ...
            f"brick_protocol/support/recording-derived write-policy fact carrying {expected_reason!r} "
...
def _check_adapter_capability_observation_out_of_scope(label: str) -> None:
    ...
    # brick_protocol/support/operator/write_observation INTO the Brick axis
    ...
    if "brick_protocol/support/connection/agent_resources.py" not in outside:
        raise ProfileError(
...
def _check_adapter_capability_no_write_observation_without_scope(label: str) -> None:
    from brick_protocol.support.operator.write_observation import _write_adapter_observation_before
...
def _native_grant_policy_resource(policy_ref: str) -> Mapping[str, Any]:
    ...
    return {
        "ref": policy_ref,
        "kind": "tool_policy",
        "path": f"brick_protocol/agent/tool_policies/{policy_ref.removeprefix('tool-policy:')}.yaml",
        "data": {
...
def _check_adapter_capability_write_scope_on_read_only_brick_rejected(
    ...
    """
    from brick_protocol.support.operator.plan_validation import _validate_declared_step_write_scope
...
def _check_adapter_capability_legacy_write_need_graph_row_key_rejected(
    ...
    """
    from brick_protocol.support.operator.plan_graph import _graph_brick_agent_rows
...
def _check_adapter_capability_silent_write_grant_rejected_single_step(
    ...
    """
    from brick_protocol.support.operator.run import run_building_once
