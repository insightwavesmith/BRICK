...
WRITE_SCOPE_MATCHER_CONSUMERS = (
    ("brick_protocol/support/operator/assembly.py", "_write_path_covered_by"),
    ("brick_protocol/support/operator/task_order_preflight.py", "_covered"),
)
...
def _write_scope() -> Mapping[str, Any]:
    return {
        "allowed_paths": ["brick_protocol/support/checkers/**"],
        "forbidden_paths": [
            ".git/**",
            "*.pem",
            "*.key",
...
def _repo_with_agent_preferred_model_omitted(repo: Path, role: str, root: Path) -> Path:
    ...
    source_agent = repo / "brick_protocol" / "agent"
    probe_agent = probe_repo / "brick_protocol" / "agent"
    probe_agent.mkdir()
...
def _graph_declaration_fire(repo: Path) -> tuple[str, ...]:
    graph_scope = {
        "allowed_paths": ["brick_protocol/support/checkers/**", "brick_protocol/support/operator/**"],
        "forbidden_paths": [".git/**"],
    }
    declaration = {
        ...
        "nodes": [
            ...
            },
            {
                ...
                "work_statement": (
                    ...
                    "## Deliverables\n"
                    "D1: edit brick_protocol/support/checkers/check_assembly_equivalence.py\n\n"
                    "## Done Criteria\n"
                    "Return observed evidence only."
                ),
                "write_scope": {
                    "allowed_paths": ["brick_protocol/support/checkers/check_assembly_equivalence.py"],
                    "forbidden_paths": [".git/**"],
                },
            },
    ...
    cli_source = (_REPO_ROOT / "brick_protocol" / "support" / "operator" / "cli.py").read_text(encoding="utf-8")
    ...
    def mutation_probe() -> None:
        ...
        mutated["nodes"][2]["write_scope"] = {
            "allowed_paths": ["brick_protocol/brick/spec.py"],
            "forbidden_paths": [".git/**"],
        }
...
def _node_write_scope_fire(repo: Path) -> tuple[str, ...]:
    graph_scope = {
        "allowed_paths": ["brick_protocol/support/checkers/**", "brick_protocol/support/operator/**"],
        "forbidden_paths": [".git/**", "brick_protocol/support/operator/secret-*"],
    }
    node_scope = {
        "allowed_paths": ["brick_protocol/support/checkers/check_assembly_equivalence.py"],
        "forbidden_paths": [".git/**", "brick_protocol/support/operator/secret-*"],
...
def _llm_alias_fire(repo: Path) -> tuple[str, ...]:
    ...
    for root_name in ("brick_protocol/brick", "brick_protocol/agent", "brick_protocol/support"):
    ...
    if declaration_paths != ["brick_protocol/support/operator/provider_registry.py"]:
        raise AssemblyEquivalenceError(
            "llm alias declaration must exist only in brick_protocol/support/operator/provider_registry.py; "
    ...
    return (
        ...
        "llm alias green: LLM_ALIAS_DECLARATIONS declaration body is brick_protocol/support/operator/provider_registry.py only.",
    )
...
def _approval_runner():
    ...
    def _runner(args: Sequence[str], cwd: Path, timeout_seconds: int):
        ...
        payload = {
            ...
            "checked_sources": ["brick_protocol/support/operator/assembly.py", "brick_protocol/support/operator/onboard.py"],
            "regression_risks": [],
            ...
            "evidence_used": ["brick_protocol/support/operator/onboard.py", "brick_protocol/support/operator/assembly.py"],
