#!/usr/bin/env python3
"""Seal brick_protocol/link/gate.py against re-computing Brick MEASUREMENT.
...
by the Brick axis at ``brick_protocol/brick/comparison.py`` (the ``brick_comparison`` crossing,
...
that mixed measurement computation into ``brick_protocol/link/gate.py``.
...
This checker is support evidence only. It parses brick_protocol/link/gate.py instead of
...
_GATE_REL = Path("brick_protocol/link/gate.py")

# The Brick MEASUREMENT crossing (crossing_registry.yaml: brick_comparison),
# owned by the Brick axis at brick_protocol/brick/comparison.py. The gate must not import these
...
PROOF_LIMIT = (
    ...
    "sufficiency. It proves only that brick_protocol/link/gate.py homes no Brick measurement "
    "computation (measurement = Brick at brick_protocol/brick/comparison.py)."
)


class LinkGateMeasurementSeparationError(ValueError):
    """Raised when brick_protocol/link/gate.py imports or recomputes Brick measurement."""
        ...
        if isinstance(node, ast.Import):
            for alias in node.names:
                ...
                if any(token in name for token in _FORBIDDEN_IMPORT_TOKENS):
                    violations.append(
                        "gate.measurement-separation: brick_protocol/link/gate.py imports the "
                        f"Brick measurement module {name!r} "
                        "(measurement is Brick-owned at brick_protocol/brick/comparison.py; the "
            ...
            if any(token in module_name for token in _FORBIDDEN_IMPORT_TOKENS):
                violations.append(
                    "gate.measurement-separation: brick_protocol/link/gate.py imports from the "
                    f"Brick measurement module {module_name!r} "
                    "(measurement is Brick-owned at brick_protocol/brick/comparison.py; the gate "
    ...
    for node in ast.walk(module):
        ...
        if isinstance(node, ast.Call):
            ...
            if callee is not None:
                violations.append(
                    "gate.measurement-separation: brick_protocol/link/gate.py constructs the "
        ...
        # (b) Defining the measurement contract or a measurement factory here.
            ...
            violations.append(
                "gate.measurement-separation: brick_protocol/link/gate.py defines a "
                f"{_MEASUREMENT_SYMBOL} class "
                "(the Brick measurement contract is owned by brick_protocol/brick/comparison.py)"
            )
        elif isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            if node.name in _MEASUREMENT_FACTORY_NAMES:
                violations.append(
                    "gate.measurement-separation: brick_protocol/link/gate.py defines the Brick "
                    f"measurement factory {node.name!r} "
                    "(measurement derivation is owned by brick_protocol/brick/comparison.py)"
                )
    return violations
...
def _assert_mutation_red() -> str:
    ...
    into brick_protocol/link/gate.py, the live ``check_gate_source`` REDs exactly as this probe
...
def check(repo: Path) -> list[str]:
    ...
    if violations:
        raise LinkGateMeasurementSeparationError(
            "brick_protocol/link/gate.py homes Brick measurement computation:\n"
            + "\n".join(f"- {v}" for v in violations)
        )
    mutation_line = _assert_mutation_red()
    return [
        "link gate measurement-separation green: brick_protocol/link/gate.py imports no Brick "
        ...
        "(measurement = Brick at brick_protocol/brick/comparison.py, judgment = Link gate).",
        mutation_line,
        PROOF_LIMIT,
    ]
...
def parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Support-evidence checker that seals brick_protocol/link/gate.py against importing "
