...
vocabulary as ONE data table — ``GATE_REGISTRY`` in ``brick_protocol/link/spec.py`` (one row per
declarable gate: ref + concept_token + required_return_fields + placement +
disposition). Every other gate fact DERIVES from it:

  * ``brick_protocol/link/gate.py`` derives ``DECLARED_GATE_REFS`` + the disposition partitions +
...
vocabulary lived as a literal 4-tuple in ``brick_protocol/link/gate.py`` and the placement rule
lived as a hand-authored ``if qa_row and "strict-evidence" in tokens`` ladder in
``brick_protocol/support/operator/composition_gate_translation.py``; a future edit could grow a
...
Two AST rules (NO import of the scanned modules; pure structural evidence):
  ...
  everywhere except ``brick_protocol/link/spec.py`` (where the GATE_REGISTRY rows legitimately
...
The single source ``brick_protocol/link/spec.py`` is also positively asserted: ``GATE_REGISTRY``
...
_SPEC_REL = Path("brick_protocol/link/spec.py")
...
_MATERIALIZER_REL = Path("brick_protocol/support/operator/composition_gate_translation.py")

# A gate ref is a string literal beginning with this prefix. The registry rows in
# brick_protocol/link/spec.py enumerate them once; nowhere else may a COLLECTION of 2+ of them
...
PROOF_LIMIT = (
    ...
    "rule are enumerated/authored in exactly one place — brick_protocol/link/spec.GATE_REGISTRY "
...
def check_spec_single_source(module: ast.Module) -> list[str]:
    """Positively assert the single source REALLY lives in brick_protocol/link/spec.py."""
    ...
    if registry_refs < _ENUMERATION_MIN:
        violations.append(
            "gate-registry single-source: brick_protocol/link/spec.py GATE_REGISTRY does not "
    ...
    if _function_def(module, _PLACEMENT_READER) is None:
        violations.append(
            "gate-registry single-source: brick_protocol/link/spec.py defines no "
...
def check_no_vocab_enumeration(rel: Path, module: ast.Module) -> list[str]:
    ...
    for line in find_gate_ref_enumerations(module):
        violations.append(
            ...
            "is owned ONCE by brick_protocol/link/spec.GATE_REGISTRY; derive DECLARED_GATE_REFS / "
...
def check_materializer_delegates_placement(module: ast.Module) -> list[str]:
    ...
    if not _calls_name(fn, _PLACEMENT_READER):
        violations.append(
            ...
            "the materializer must READ placement from brick_protocol/link/spec.GATE_REGISTRY, "
            "not author it"
        )
...
def check(repo: Path) -> list[str]:
    ...
    return [
        ...
        "enumerated ONCE in brick_protocol/link/spec.GATE_REGISTRY (brick_protocol/link/gate.py derives "
...
def parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "placement rule to brick_protocol/link/spec.GATE_REGISTRY (one row per gate; nothing "
            "re-stated elsewhere)."
        )
    )
