...
assignment). The code trees brick_protocol/support/, brick_protocol/brick/, brick_protocol/agent/, brick_protocol/link/ are scanned.
...
# function, one home — brick_protocol/support/recording/capture.py). The same anchor rules
...
SCAN_DIRS = ("brick_protocol/support", "brick_protocol/brick", "brick_protocol/agent", "brick_protocol/link")
...
def find_violations(repo: Path) -> tuple[list[str], int]:
    ...
    if not seam_defs:
        violations.append(
            ...
            "seam must exist exactly once (brick_protocol/support/recording/capture.py)"
        )
