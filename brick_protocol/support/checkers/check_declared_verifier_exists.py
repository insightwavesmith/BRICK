...
in that list whose file does NOT exist in ``brick_protocol/support/checkers/`` is a pure
...
def find_violations(repo: Path) -> tuple[list[str], int, int]:
    checker_dir = repo / "brick_protocol" / "support" / "checkers"
    violations: list[str] = []
    declared = 0
    inspected = 0
        ...
        for line_no, name in entries:
            ...
            if not (checker_dir / name).is_file():
                ...
                violations.append(
                    ...
                    f"brick_protocol/support/checkers/{name}"
                )
