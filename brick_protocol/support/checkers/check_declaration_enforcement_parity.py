...
* every narrow "future/later gate consumer" declaration in Brick template YAML
  is registered in ``brick_protocol/brick/templates/enforcement-ledger.yaml``, and every ledger
...
from brick_protocol.agent.return_fact import (  # noqa: E402
    ALWAYS_SECRET_KEYS,
    TOP_LEVEL_VERDICT_KEYS,
)
from brick_protocol.support.checkers.lib.yaml_subset import parse_yaml_subset  # noqa: E402


BRICK_RETURNS_ROOT = Path("brick_protocol/brick/templates/bricks")
BRICK_RETURN_FILENAME = "return.yaml"
LEDGER_REL = Path("brick_protocol/brick/templates/enforcement-ledger.yaml")
DECLARATION_SCAN_ROOT = Path("brick_protocol/brick/templates")
DECLARATION_SCAN_EXCLUDES = {
    "brick_protocol/brick/templates/enforcement-ledger.yaml",
}
DECLARATION_PHRASES = (
...
def check(repo: Path) -> list[str]:
    ...
    if not declarations:
        raise DeclarationEnforcementParityError(
            "no gate-pending declaration phrases found under brick_protocol/brick/templates"
        )
