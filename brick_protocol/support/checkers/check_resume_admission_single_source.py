...
points in ``brick_protocol/support/operator/walker_resume.py`` -- the live resume path
...
(``brick_protocol/support/checkers/lib/rule_runners.py``) are whole-file substring
...
def _refusal_probe_count() -> int:
    ...
    _assert_refuses(
        walker,
        "claims-file-absent",
        "required claim_trace brick_protocol/agent/returned_claims.json",
        evidence=gate_pause_evidence,
        disposition={"disposition_action": "forward"},
        declared_plan=plan,
        recorded_returns_loader=ok_returns,
