...
``brick_protocol/support/operator/report_sinks.py`` / ``reporter.py``). If those keys are absent
...
``resume_building_plan`` / ``run_building_once`` in ``brick_protocol/support/operator/run.py``,
...
INJECTION SCOPE (codex review 0619, MAJOR-5 -- narrowed):
  ...
  - PROVIDER keys (``GEMINI_API_KEY`` / ``GOOGLE_API_KEY``) ARE injected into the
    ...
    ``os.environ`` (``brick_protocol/support/connection/agent_adapter.py``) with no threaded-env
