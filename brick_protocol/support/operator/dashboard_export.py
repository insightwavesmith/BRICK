...
DEFAULT_DASHBOARD_PUBLIC_DATA_PATH = Path("brick_protocol/support/dashboard/public/dashboard-data.json")
...
def dashboard_export_packet(
    *,
    ...
    # 에이전트 로스터 (brick_protocol/agent/objects/*.yaml 은 JSON)
    agents = []
    for path in sorted(glob.glob(str(repo / "brick_protocol" / "agent" / "objects" / "*.yaml"))):
        try:
    ...
    # 브릭 카탈로그
    bricks = []
    for dpath in sorted(glob.glob(str(repo / "brick_protocol" / "brick" / "templates" / "bricks" / "*") + os.sep)):
...
def bake_dashboard_data_json(
    *,
    ...
    """Bake the read-side dashboard seed into brick_protocol/support/dashboard/public.

    This is a synchronous brick_protocol/support/operator projection verb only. It computes
...
def _main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--bake-public",
        action="store_true",
        help="write brick_protocol/support/dashboard/public/dashboard-data.json from dashboard_export_packet",
    )
    parser.add_argument(
        "--building-id",
        default="",
