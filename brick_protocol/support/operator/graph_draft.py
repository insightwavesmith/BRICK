...
# glob 전수, bare-dir 금지. (brick_protocol/brick/spec.py:257-273; 0612 write_scope 문법 함정.)
# ---------------------------------------------------------------------------
def _normalize_scope_entry(entry: str, repo_root: Path, rows: list[dict[str, str]], *, field_name: str) -> str:
    ...
    stem = text.rstrip("/")
    c2_stem = {
        "support": "brick_protocol/support",
        "brick": "brick_protocol/brick",
        "agent": "brick_protocol/agent",
        "link": "brick_protocol/link",
    }.get(stem, stem)
    if c2_stem != stem:
        rows.append(
            {
                "rule_id": "rule5-c2-root-rebased",
                "decision": f"{field_name} {entry!r} → {c2_stem!r}",
                "basis": "C2 import-unify physical roots live under brick_protocol/",
            }
        )
        stem = c2_stem
        ...
        rows.append(
            {
                ...
                "basis": "brick_protocol/brick/spec.py:264 rejects a bare-dir write entry",
            }
        )
        return fixed
    return text
