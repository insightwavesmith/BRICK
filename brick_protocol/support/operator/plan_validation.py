...
def _comparison_with_artifact_grounding(
    ...
    # (brick_protocol/brick/spec.apply_artifact_grounding_completeness). Behavior-identical to the
...
def _task_source_ref_from_plan(
    ...
    *,
    ...
    if len(parts) >= 2 and parts[:2] == ("brick", "tasks"):
        raise ValueError("task_source_ref must not point at brick_protocol/brick/tasks active task instances")
...
# judgment and now live on the BRICK axis (brick_protocol/brick/spec.py). Support re-imports the
...
def _movement_and_target_from_link_row(link_row: Mapping[str, Any]) -> tuple[str, str]:
    ...
    # brick_protocol/link/movement.py MOVEMENT_LITERALS (forward/reroute), at preflight.
...
# (key-sets, prefixes, evidence spec) is single-sourced in brick_protocol/link/spec.py; these
...
def _validate_route_replay_author_ref(value: str) -> None:
    ...
    if (
        ...
        or "brick_protocol/support/run" in lowered
        or "support:" in lowered
        or "brick_protocol/support/" in lowered
    ):
