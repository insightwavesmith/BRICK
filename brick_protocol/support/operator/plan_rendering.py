...
SPLIT_SHAPE_CATALOG_PATH = Path("brick_protocol/brick/templates/shapes/catalog.yaml")
BRICKS_SPEC_DIR = Path("brick_protocol/brick/templates/bricks")
...
PRESETS_SPEC_DIR = Path("brick_protocol/brick/templates/presets")
...
def _step_templates_from_bricks(repo: Path) -> dict[str, dict[str, Any]]:
    ...
    for path in _brick_spec_paths(repo):
        ...
        path_kind = path.parent.name if path.name in {"brick.md", "brick_protocol.brick.md"} else path.stem
        if kind != path_kind:
...
def _validate_declared_plan_projection(plan: Mapping[str, Any]) -> None:
    from brick_protocol.support.operator.plan_validation import (  # noqa: PLC0415
