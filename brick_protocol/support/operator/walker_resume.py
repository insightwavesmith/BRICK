...
transition-lifecycle author-prefix contract of brick_protocol/link/transition.py to validate the
disposition author).
"""
...
# (brick_protocol/link/transition.py DISPOSITION_ACTIONS is untouched -- Movement stays
...
def resume_admission_decision(
    *,
    ...
    if expected_replay_counts and not returned_claims_present_loader():
        raise ValueError(
            "resume corrupt evidence: required claim_trace brick_protocol/agent/returned_claims.json "
...
def _merge_raw_manifest_entry(
    ...
    *,
    ...
    for entry in entries:
        ...
        entry.update(
            {
                "source": "brick_protocol/support/operator/walker_resume.py paper stop disposition record",
