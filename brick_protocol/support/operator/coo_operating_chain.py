...
def operator_loop_packet(
    *,
    ...
    This helper does not call ``brick_protocol/support/operator/run.py``. The declared
    ...
    *,
    ...
    return {
        ...
        "design_context_observation": {
            ...
            "toolkit_ref": "brick_protocol/support/connection/building_design_toolkit.py::render_building_design_context",
        ...
        "active_plan_observation": {
            ...
            "plan_is_brick_owned": _rel(repo, plan_path).startswith("brick_protocol/brick/building_plans/"),
        },
...
def _coo_operating_chain_mcp_observation(repo: Path) -> Mapping[str, Any]:
    try:
        from brick_protocol.support.connection.mcp_projection import (  # noqa: PLC0415
...
def _coo_operating_chain_gap_notes(
    *,
    ...
    if not has_task_intake_skill:
        notes.append(gap_note(issue="COO Agent Object does not reference skill:task_intake", evidence_refs=("brick_protocol/agent/objects/coo.yaml",)))
    if not bool(mcp_observation.get("resource_present")) or not bool(mcp_observation.get("tool_present")):
        notes.append(gap_note(issue="COO operating chain MCP context is not fully exposed", evidence_refs=("brick_protocol/support/connection/mcp_projection.py",)))
