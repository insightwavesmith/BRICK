...
single-sourced in brick_protocol/link/spec.py via ``translate_gate_concept``; this support
...
def _materializer_profile_gate_translations(
    ...
    *,
    ...
    single-source data table via ``gate_placement_for_row`` (brick_protocol/link/spec.py
...
def _materializer_human_gate_hold_policy() -> list[Mapping[str, Any]]:
    ...
    brick_protocol/link/gate.yaml meaning of link-gate:human is "human disposition evidence
...
def stamp_declared_portfolio_closure_gates(
    ...
    *,
    ...
    EXISTING engine gate machinery (run.py / brick_protocol/link/gate.py untouched).
