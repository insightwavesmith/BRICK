...
_DESIGN_RESOURCE_PATHS = {
    "task_source_template": Path("brick_protocol/brick/templates/tasks/source-template.md"),
    "human_ai_design_contract": Path("brick_protocol/brick/templates/building-design-contract.yaml"),
}
from brick_protocol.support.connection.secret_text import contains_raw_secret_text
_PROOF_LIMITS = (
    "support design context evidence only",
    "Brick meaning remains owned by brick_protocol/brick/",
...
_BOARD_LINK_CATALOG_PATHS = {
    "movement": Path("brick_protocol/link/movement.yaml"),
    "carry": Path("brick_protocol/link/carry.yaml"),
    "transition": Path("brick_protocol/link/transition.yaml"),
}
_BOARD_PROOF_LIMITS = (
    "support catalog projection evidence only",
    "the board unifies the four catalogs for reading; it changes none of them",
    "Brick / Agent / Link meaning stays owned by brick_protocol/brick/ , brick_protocol/agent/ , brick_protocol/link/",
...
def render_building_board(repo_root: str | Path | None = None) -> dict[str, Any]:
    ...
    * ``shapes`` -- the declared Building Shape menu refs.
    ...
    return {
        "kind": "building-board",
        "source": "brick_protocol/brick/ + brick_protocol/agent/ + brick_protocol/link/",
...
def _board_links(repo: Path) -> dict[str, Any]:
    ...
    Movement literals come from ``brick_protocol/link/movement.yaml`` (forward / reroute); the
...
def _board_movement_literals(movement_doc: Mapping[str, Any]) -> list[str]:
    """The admitted Movement literals declared in brick_protocol/link/movement.yaml."""
