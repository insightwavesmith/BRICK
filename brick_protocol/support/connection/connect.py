...
own ``__file__`` bootstrap self-adds ``brick_protocol/support/import_identity`` on a bare host
...
brick_protocol/support/connection/*), writes NO file, stores NO credential, calls NO provider,
...
_MCP_SERVER_REL = "brick_protocol/support/connection/mcp_projection.py"


def repo_root_from_here() -> Path:
    """Compute the repo root from this file's location.

    ``brick_protocol/support/connection/connect.py`` -> parents[2] is the repo root, exactly
