...
if [ "${1:-}" = "--ship" ]; then
  ...
  ( cd "$REPO" && PYTHONPATH="$REPO/support/import_identity" python3 brick_protocol/support/checkers/check_profile.py --all ) > /tmp/coo-ship-sweep.log 2>&1
  RC=$?
...
if [ "${1:-}" = "--land" ]; then
  ...
  ( cd "$REPO" && PYTHONPATH="$REPO/support/import_identity" python3 brick_protocol/support/checkers/check_profile.py --all ) > /tmp/coo-land-sweep.log 2>&1
  RC=$?
...
for P in $PROFILES; do
  ( cd "$GATE" && PYTHONPATH="$GATE/support/import_identity:$GATE" python3 brick_protocol/support/checkers/check_profile.py --profile "$P" ) > "/tmp/coo-gate-$B-$P.log" 2>&1
  RC=$?
  say "profile $P: rc=$RC"
...
( cd "$GATE" && PYTHONPATH="$GATE/support/import_identity" python3 brick_protocol/support/checkers/check_profile.py --all ) > "/tmp/coo-gate-$B-all.log" 2>&1
RC=$?
