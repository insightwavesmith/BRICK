...
#  brick_protocol/support/onboarding/install.sh from the checkout)
...
preflight_gh() {
    ...
    if [ -z "$repo_slug" ]; then
        printf '%s\n' \
            "  ✗ 받을 저장소 미지정 — 지금 치세요: BRICK_REPO={OWNER}/BRICK-dist sh brick_protocol/support/onboarding/install.sh" >&2
        return 1
    fi
