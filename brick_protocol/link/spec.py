...
#   * brick_protocol/link/gate.py derives ``DECLARED_GATE_REFS`` + the per-gate required-return
...
# literal list). brick_protocol/link/gate.py imports this so support's ``DECLARED_GATE_REFS``
# (re-exported from brick_protocol/link/gate.py) stays the byte-identical 4-tuple it always was.
...
# ---------------------------------------------------------------------------
# LINK ROW + ENVELOPE KEYS (moved from brick_protocol/support/operator/primitives.py at E2/S4).
...
# brick_protocol/support/operator/primitives.py at E2/S4 mirror M12). Each is the member-set the
